...
source_of_truth_for:
  ...
  - integration gates
  - follow_up_dispatch_policy and marketing_contact_policy separation
depends_on:
...
| address book | Customer & Identity | planned | identity, address contract | customer-address-contract | future_release_not_scheduled | yes | address ownership, confirmation, no direct writes ajenos |
| marketing_contact_policy | Marketing | future | identity, consent, contact policy | contact policy contract | outside current MVP | yes | campanas, audiencias, suppression, contacto masivo, preferencias de marketing, consent, frequency caps, authority model |
...
| follow-up | Commercial Runtime | implemented_partial | scheduling, outbox, policy | follow-up-decision-policy | ACS-R1-05 | yes | idempotency, cancellation, policy checks |
| follow_up_dispatch_policy | Commercial Runtime | implemented_partial | scheduling, contact identity, consent evidence | follow-up-decision-policy | ACS-R1-05 | yes | consent, opt-out, quiet hours, timezone, cooldown, frequency cap, max attempts, identity conflict, human takeover, AI paused, conversation/opportunity terminal |
...
| analytics events | Analytics | planned | canonical IDs, domain events | customer-lifecycle-event-contract | future_release_not_scheduled | yes | versioned events, replayability, projection ownership |
| commercial metrics | Analytics | planned | analytics events | metrics contract | future_release_not_scheduled | yes | metric definitions, projection ownership |
| agent performance metrics | Analytics | planned | analytics events, execution logs | metrics contract | future_release_not_scheduled | yes | stable metrics and replayability |
...
| marketing_contact_policy | Marketing | Commercial Runtime, Voice, Operator CRM | Marketing policy service only | everyone else | contact policy contract | future marketing store |
| follow_up_dispatch_policy | Commercial Runtime / Platform & Integrations | Commercial Runtime, Operator CRM | Commercial Runtime service only | Operator CRM, Analytics | follow-up-decision-policy | `crm_agent_actions.policy_status` (evaluation itself not yet a dedicated table) |
...
- marketing_contact_policy -> Marketing execution
- marketing_contact_policy -> Voice initiation
- Commercial Runtime -> follow_up_dispatch_policy
- `follow_up_dispatch_policy` es independiente de `marketing_contact_policy`: Marketing no puede bloquear el follow-up comercial 1:1 del MVP, y Commercial Runtime no puede ejecutar campanas ni usar `marketing_contact_policy`
...
- consentimiento para follow-up (`consent.allowFollowUp` o equivalente)
- opt-out
- quiet hours
- timezone
- cooldown
- frequency cap (por customer y por opportunity)
- max attempts
- identity conflict
- human takeover
- AI paused
- conversation/opportunity terminal

Estado real de cada gate (que esta enforced-at-dispatch vs. solo tipo/planner/shadow): [Follow-up runtime reconciliation](../audits/follow-up-runtime-reconciliation.md).
...
- `marketing_contact_policy` (Marketing) no puede bloquear el follow-up comercial 1:1 del MVP (`follow_up_dispatch_policy`, Commercial Runtime); Commercial Runtime no puede ejecutar campanas ni usar `marketing_contact_policy` para contacto masivo.
