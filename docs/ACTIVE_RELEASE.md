...
`ACS-R1-05-T03` (cerrada, commit `6558f29`) endurecio el runtime canonico de follow-up (`sales-consultative` -> `crm_agent_actions` -> `runFollowupTick` -> `runNativeAutonomousCycle`) sin tocar el planner T01 ni la dispatch policy T02. `selectDueFollowUps` ahora selecciona tres grupos disjuntos de candidatas, cada uno con su propio CAS: `planned` due (sin cambio), `executing` con lock vencido (`updated_at` mas antiguo que `FOLLOW_UP_STALE_EXECUTING_LOCK_SECONDS = 300`, constante nueva en `followUpWorkerPolicy.ts`) y `attempt_number < max_attempts` (P0-2), y `failed` con `attempt_number < max_attempts` (P0-3) - una fila en `max_attempts` es terminal en cualquiera de los tres grupos, sin necesidad de un valor de `status` distinto. `claimStaleExecutingFollowUp` recupera sin incrementar `attempt_number` (resume el mismo intento interrumpido por un crash, no un intento comercial nuevo); `claimFailedFollowUpRetry` incrementa `attempt_number` exactamente una vez, atomicamente dentro del propio `UPDATE` que gana el CAS. `cancelFollowUp` ahora exige `status IN ('planned', 'failed')` y retorna `{cancelled: boolean}` en vez de `void` (P1-1) - nunca sobrescribe `executing`/`executed`/`cancelled`/`requires_review`; una fila `executing` recuperada se revalida (respuesta del cliente, oportunidad terminal) despues de su propio claim, nunca antes, y se aborta via un CAS distinto (`abortClaimedFollowUp`, precondicion `status = 'executing'`) que tambien reemplaza el `cancelFollowUp` sin precondicion que el codigo original usaba para `conversation_not_found`. Probado con 24/24 tests en `tests/commercial/runFollowupTick.test.ts` (15 nuevos) contra MariaDB real (`main_management`), mas 99/99 combinados con los tests de T01/T02 sin modificar sus aserciones, mas 106/106 en regresion adicional (`actionLifecycleContract`, `actionQueueViewModel`, `agentActionQueue`, `sales-consultative*`, `outboxWorker`) y el escenario E del harness E2E (`npm run e2e:autonomous`) en `PASS` antes y despues del cambio. Detalle completo, incluida la evidencia de concurrencia CAS y el reset local de infraestructura de pruebas, en la seccion "Evidencia de cierre - ACS-R1-05-T03" de la release spec.
