...
`ACS-R1-05-T06` (cerrada, rama `acs-r1-05-t06-operational-safety`) cerro los gaps P1-2 y P1-5 de la auditoria sin tocar `ACS-R1-05-T05` (aislamiento de codigo muerto/paralelo, diferida de `in_progress` a `ready` para priorizar este incremento - sigue siendo obligatoria antes de la aceptacion formal de `ACS-R1-05-T07`, cuya fila de dependencias exige `T01 a T06`) ni ejecutar envios reales a Meta. El diagnostico previo encontro que el parseo de booleanos ya era fail-closed en todo el runtime (`commercialCycleConfig.ts#readEnvFlag`, y los lectores internos de `outboxWorker.ts`/`metaClient.ts`) - el gap P1-5 real estaba en tres scripts CLI (`scripts/autonomous-followup-worker.ts`, `scripts/autonomous-outbox-worker.ts`, `scripts/commercial-live-autonomy-run.ts`) que escribian `process.env[key] = "true"` para cada flag ausente antes de arrancar, auto-escalando en silencio todo el pipeline comercial (shadow, policy, action queue, execution gate, outbox bridge) o el envio real a Meta. Los tres ya no mutan `process.env` - solo cargan `.env`/`.env.local`; un flag ausente permanece deshabilitado y el operador debe habilitarlo explicitamente. Un modulo canonico nuevo (`lib/brain/runtime/autonomousRuntimeConfig.ts`) implementa el contrato de arranque del outbox worker: acepta unicamente `"true"`/`"false"` para `BRAIN_OUTBOX_WORKER_ENABLED`/`BRAIN_META_SEND_ENABLED`/`BRAIN_OUTBOX_WORKER_ALLOW_REAL_SEND` (cualquier otro valor lanza un error de configuracion en vez de degradar en silencio), y rechaza arrancar si los tres estan en `true` con `BRAIN_AUTONOMOUS_TEST_WA_IDS` vacio - real send autorizado con allowlist vacia es configuracion invalida durante el piloto, nunca "enviar a cualquiera". Para P1-2, los dos unicos sitios que persistian `error.message`/`errorMessage` crudo en `crm_agent_actions.failure_reason` (`runFollowupTick.ts`, `persistActionOutcome.ts#markActionFailed`) ahora pasan por una redaccion compartida nueva (`lib/brain/commercial/redactErrorMessage.ts`, mismo patron ya usado en otros 4 archivos hermanos, no tocados). No se modificaron `outboxWorker.ts`, `metaClient.ts` ni `app/api/brain/outbox/worker/route.ts` (sus lectores ya eran fail-closed, fuera del gap P1-5 nombrado) ni `ACS-R1-05-T04`. Probado con 13 tests puros nuevos (`tests/brain/autonomousRuntimeConfig.test.ts`, `tests/commercial/redactErrorMessage.test.ts`) y verificado sin regresiones contra las suites DB-backed que ejercitan el codigo modificado (`runFollowupTick.test.ts` 31/31, `deliveryOutcomeProjection.test.ts` + `canonicalOutboxWriter.test.ts` 33/33) - los 18 fallos preexistentes en la suite completa se confirmaron identicos contra `develop` HEAD via `git stash`, en areas no tocadas por esta tarea. El runtime de follow-up sigue sin `operational: verified`. Detalle completo en la seccion "Evidencia de cierre - ACS-R1-05-T06" de la release spec.
