...
`record_customer_interest` sigue sin registrar en el Gateway y sin efecto operacional. Customer 360 sigue sin recibir escritura de ninguna de estas operaciones; su lectura esta detras de una compuerta de acceso explicita (`contextAccess`) que no depende de estas capabilities.
...
Las tres policies puras (`lib/domains/customer-service/authority-policy.ts`) devuelven un `AuthorityDecision` comun (`allowed | missing_information | denied | requires_consent | requires_human`). La IA nunca decide autoridad - solo propone; la policy evalua precondiciones sobre evidencia estructurada real.
...
Esta tabla describe la autoridad conceptual (contrato seccion 9). El Capability Gateway ya tiene registradas las tres capabilities desde `T06`, pero su campo `authority` es binario y no modela directamente `policy`/`requires_consent`; las tres quedaron registradas como `authority: autonomous` porque el gate real corre dentro de `execute()`.
...
4. El `idempotencyKey` lo genera la service layer (`customer-service:<op>:<capabilityExecutionId>`).
