...
## 23.11. P7.10 - Mutation Semantics & Confirmation Boundary

**P7.10 CLOSED - `CONSEQUENCE_STATEMENT_SUFFICIENT` (senal preregistrada; `reversibleSemanticsSupported = false`).** Detalle, preregistro, resultados, analisis exploratorio y limitaciones: `docs/audits/r3-p7-10-mutation-semantics-confirmation-boundary.md`. Sin P8, sin MCP, sin cambios de R3/MS/Gateway/CommercialWork/DB/schema/capabilities de produccion, sin tocar la semantica de `quantity`.

**Diseno.** Tres superficies model-facing de `select_products` (S0 actual; S1 = primera oracion reemplazada por una declaracion de consecuencia: estado de trabajo provisional y reversible, sin orden/pago/checkout/reserva, mas la instruccion de no pedir confirmacion adicional solo para guardarlo; S2 = S1 mas `useWhen`/`doNotUseWhen` sin framing de compra/compromiso) sobre el MISMO harness autonomo de P7.8-R/P7.9; corpus `r3-p7-10.v1` de 46 escenarios (D declarativo, I imperativo, Q quote, N informativo, F follow-up, R reemplazo) x 3 corridas x 3 brazos = 414, interleaved; clasificador de confirmacion con golden de 70 frases y freeze por hash verificado por el script.

**Resultado (414 corridas, 0 fallos de harness, batch `p7-10-2026-09-21T22-24-07-790Z-live`).** Declarativo (36 turnos por brazo): seleccion 19.4% -> 66.7% -> 58.3% y confirmacion innecesaria 69.4% -> 19.4% -> 22.2% (S0 -> S1 -> S2). S0->S1 +47.2 pp / -50.0 pp; S1->S2 -8.3 pp / +2.8 pp (sin mejora); S0->S2 +38.9 pp / -47.2 pp pero insegura por la regla de Gateway rejection (+5.2 pp, compuesta por `create_quote` con Quote Service BLOCKED, `search_products_by_semantics` `registry_mismatch` y `explore_catalog` `invalid_response`, no por `select_products`). Imperativo 66.7 / 95.8 / 79.2%; quote 58.3 / 87.5 / 87.5%; follow-up 80 / 80 / 80% (F01 excluido por la regla congelada: el turno 1 nombro ambos productos); reemplazo 75 / 83 / 92%. Controles informativos: 0/24 sobre-mutacion en los tres brazos; wrongQuantity, wrongProduct y selectionCorruption 0; sensibilidad sin D03/D05 no cambia la conclusion. Latencia por llamada sin cambio (p95 ~1.6 s); tokens de entrada por turno +11% (S1) y +13% (S2) por mas rondas de tool, no por el texto.

**Lectura.** El modelo pedia "¿confirmas?" porque leia `select_products` como un compromiso mas pesado de lo que es. Declarar que es estado provisional y reversible (S1) basta en este banco; quitar ademas el framing distribuido de `useWhen`/`doNotUseWhen` (S2) no agrega. El residuo ya no es confirmacion redundante sino: "una/un" leido como articulo (pide cantidad), la ambiguedad "Pro" a secas (D03/D05) y "pedir comuna antes de guardar" (F03).

**Limitaciones que hay que citar con el resultado.** S1 agrupa "informar la consecuencia" e "instruccion explicita de no confirmar" (no separables); un modelo, harness sintetico de dos productos, 36 turnos D por brazo con IC amplios; no es evidencia de produccion ni un cambio de contrato de produccion.

**Siguiente por evidencia (no iniciado):** variante consecuencia-sin-instruccion y replica de S0 vs S1 con otro corpus/semilla y un segundo modelo antes de proponer cambio de contrato real; decidir con negocio "una/un" como cantidad 1 y el orden comuna vs guardar seleccion; no seguir reescribiendo `useWhen`/`doNotUseWhen`.

## 23.12. P7.11 - Confirmation Boundary Replication & Realistic Intent Validation

**P7.11 CLOSED - `S1_REPLICATED_AND_GENERALIZES` (senal preregistrada).** Detalle, preregistro, resultados, analisis exploratorio y limitaciones: `docs/audits/r3-p7-11-confirmation-boundary-replication.md`. Sin P8, sin MCP, sin DeepSeek Pro, sin comparacion de modelos, sin cambios de R3/MS/Gateway/CommercialWork/DB/schema/capabilities de produccion, sin tocar la semantica de `quantity`.

**Diseno.** Replica independiente de P7.10 (23.11): corpus NUEVO `r3-p7-11.v1` (48 escenarios, 72 turnos, grupos A declarativo directo / B multi-turn / C imperativo / D quote / E informativo / F correccion-reemplazo; ningun mensaje copiado de P7.10), lenguaje comercial mas natural, mas multi-turn que P7.10. R0 y R1 NO se recrean: son `S0`/`S1` de P7.10 importadas sin tocar (byte-identicas, test-verificado); S2 no se usa. Mismo harness autonomo de P7.8-R/P7.9/P7.10, mismo modelo/config, mismo clasificador de confirmacion y golden de 70 frases de P7.10 reutilizados sin cambios (0 discrepancias). En el grupo B el producto siempre lo nombra el cliente en el turno 1 o 2 (nunca se infiere de la respuesta del asistente, a diferencia del F01 de P7.10); el turno 3 declara solo la cantidad y es el unico turno analizado.

**Resultado (288 corridas, 3 por celda, 0 fallos de harness, batch `p7-11-2026-09-22T02-33-35-337Z-live`).** Grupo A (36 turnos por brazo): `actionableSelectionRate` 58.3% -> 86.1% (+27.8 pp) y `unnecessaryConfirmationRate` 25.0% -> 2.8% (-22.2 pp) - ambos superan el margen de soporte de 20 pp. Grupo B multi-turn (36 turnos por brazo): `selectionAfterFactsCompleteRate` 25.0% -> 72.2% (+47.2 pp), muy por sobre el margen de generalizacion de 15 pp, sin degradacion. Seguridad intacta: over-mutation informativa 0/24 en ambos brazos, wrongQuantity 0/0, wrongProduct mejora (3.2% -> 1.1%), selectionCorruption 0/144 en ambos. Grupo C (imperativo, control positivo) 72.2% -> 83.3%; grupo D (quote) 61.1% -> 83.3%.

**Deuda especifica (residuo negativo, exploratorio, fuera de la senal preregistrada).** El grupo F (correccion/reemplazo de una seleccion durable ya sembrada) EMPEORA con R1: `selectionRate` 75.0% -> 50.0% y `correctDurableRate` 58.3% -> 41.7%. Causa visible en `failures.json`: R1 concentra mas rechazos `GATEWAY_REJECTION` con secuencia `select_products:blocked` (sin `get_product_details` previo en el turno) - el evidence gate bloquea porque el producto de reemplazo no fue "observado" en esa conversacion. Lectura exploratoria: la instruccion de R1 ("no pidas confirmacion adicional, solo actualiza la seleccion") parece empujar al modelo a llamar `select_products` directo para el producto nuevo sin volver a consultar `get_product_details`. No esta cubierto por los criterios de seguridad preregistrados de esta fase (Gateway rejection no es gate aqui) y no afecta la senal, pero limita cualquier lectura de "R1 mejora todo" y debe resolverse antes de adoptar S1/R1 en flujos de correccion de produccion.

**Siguiente por evidencia (no iniciado):** P7.12 real-traffic-derived validation (regla de decision preregistrada, seccion 42 de la tarea); en paralelo, investigar si el texto S1 necesita una clausula que anime a re-verificar el producto (`get_product_details`) antes de reemplazar una seleccion durable existente por otro producto, para no aumentar rechazos del evidence gate en flujos de correccion.

## 23.13. P7.12 - Conversational Soak & Adversarial Stress Test

**P7.12 CLOSED - `ROBUSTNESS_FAIL` (senal preregistrada).** Detalle, preregistro, resultados, analisis exploratorio y limitaciones: `docs/audits/r3-p7-12-conversational-soak-stress.md`. Sin P8, sin MCP, sin DeepSeek Pro, sin comparacion de modelos, sin cambios de R3/MS/Gateway/CommercialWork/DB/schema/capabilities de produccion. S1 quedo FIJO (sin comparacion S0/S1): `P7.12 S1 === P7.11 R1 === P7.10 S1`, byte-identico, test-verificado.

**Diseno.** Soak conversacional continuo de 3 conversaciones persistentes e intercaladas (BUYER_CHAOTIC / BROWSER_UNCERTAIN / ADVERSARIAL_STRESS) sobre el mismo harness autonomo de P7.8-R/P7.10/P7.11, con un stress-plan congelado de 183 turnos (26 categorias de stress, >= 20 transiciones de correccion/reemplazo, 5 fallos inyectados one-shot) repetido mecanicamente 4 veces (`cycles=4`, mismo plan textual, nunca contenido nuevo) para acercarse al objetivo de duracion de la tarea sin generarlo dinamicamente.

**Hallazgo principal: colapso severo del uso de tools en conversaciones largas.** 732/732 turnos ejecutados, 0 `HARD_FAILURE`, 0 corrupcion estructural de la seleccion, 0 fuga cross-conversation - pero **54 release blockers**. Solo 24/732 turnos (3.3%) invocaron algun tool; la ultima invocacion de todo el soak fue en el turno `sequenceIndex 323` de 731 (44% del recorrido) - **los 408 turnos siguientes (55.8% del soak, el resto del ciclo 2 y los ciclos 3 y 4 completos) no volvieron a invocar NINGUN tool**, en ninguna de las 3 conversaciones. De los 84 turnos accionables con expectativa declarada, 83 (98.8%) no invocaron `select_products`; una fraccion relevante de esos (analisis exploratorio, ~31%) narro una actualizacion de la seleccion en texto sin ejecutarla ("¡Listo! Actualizo a 2 unidades...").

**Lectura.** La seguridad estructural se mantuvo intacta en todo momento (0 corrupcion, 0 fuga, over-mutation informativa 0.8%): cuando el modelo SI actuaba, actuaba de forma segura. El problema es que, pasado cierto punto de una conversacion larga y con patrones repetidos, el modelo dejo de invocar capabilities gobernadas y empezo a responder solo conversacionalmente. Los tokens de entrada crecieron de forma moderada (8k -> 22k, con un pico de 62k) - no hay evidencia de que se agotara una ventana de contexto dura; es mas compatible con una deriva de comportamiento especifica de conversaciones largas/repetidas que con un limite tecnico.

**Limitacion importante (declarada, no se esconde).** Este comportamiento se observo en UN soak principal y todavia no fue replicado de forma independiente. El plan repetido 4 veces (`cycles=4`) puede el mismo contribuir a la deriva de comportamiento (mensajes literalmente identicos reapareciendo varias veces en el mismo historial) - no se puede distinguir, dentro de esta sesion, entre "degradacion genuina de conversaciones largas" y "artefacto del ciclado".

**Decision de arquitectura.** NO avanzar a la comparacion final R3 vs harness nativo todavia. Antes: replicar y aislar el colapso de tool-use en conversaciones largas (conversacion unica, sin ciclado, contenido no repetido, midiendo en que punto exacto se pierde el tool-calling), y ampliar el detector de false-success-claim (como modulo de benchmark, nunca el de produccion sin autorizacion explicita) para medir el fenomeno real sin subestimarlo.

**Siguiente por evidencia (no iniciado):** replica aislada del colapso de tool-use (prioridad inmediata, antes de cualquier P7.13 previamente planificado); no tocar el evidence gate, S1 ni produccion hasta tener esa replica.
...
- No leer P7.10 (`CONSEQUENCE_STATEMENT_SUFFICIENT`, 23.11) como autorizacion para cambiar el contrato real de `select_products`, ni como prueba de que S2 sea insegura ni de que reescribir `useWhen`/`doNotUseWhen` ayude: es una variante de benchmark (un modelo, corpus sintetico), S1 no separa "informar la consecuencia" de "instruccion de no confirmar", y el `reversibleSemanticsSupported = false` viene de rechazos de Gateway ajenos a `select_products`. Cualquier adopcion requiere replica y una release explicita.
- No leer P7.11 (`S1_REPLICATED_AND_GENERALIZES`, 23.12) como autorizacion para llevar S1 a produccion todavia: replica el efecto S0->S1 de P7.10 en un corpus nuevo y lo generaliza a multi-turn, pero sigue siendo un solo modelo/benchmark sintetico, y el grupo F (correccion/reemplazo) EMPEORA con R1 por mas rechazos del evidence gate (`select_products` sin `get_product_details` previo al reemplazar por otro producto) - esa deuda debe resolverse antes de cualquier adopcion en flujos de correccion.
- No avanzar a la comparacion final R3 vs harness nativo, ni a un P7.13 de validacion con trafico real, sin resolver primero P7.12 (`ROBUSTNESS_FAIL`, 23.13): en conversaciones largas/repetidas el modelo dejo de invocar tools por completo durante mas de la mitad de un soak de 732 turnos (ultima invocacion en el turno 323/731), narrando actualizaciones de seleccion que nunca ejecuto en una fraccion relevante de esos turnos. La seguridad estructural (corrupcion, fuga cross-conversation) se mantuvo intacta - el riesgo es puramente de ejecucion/comportamiento. El hallazgo NO esta replicado todavia (un solo soak, posible contribucion del plan repetido 4 veces) - la prioridad inmediata es una replica aislada (conversacion unica, sin ciclado, contenido no repetido) antes de cualquier otra decision de arquitectura.
