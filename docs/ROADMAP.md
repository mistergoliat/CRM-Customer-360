...
`ACS-R1-05.1`: `status: parallel_in_progress`, `critical_path: true`, `current_task: ACS-R1-05.1-T02.1`, `current_task_status: planned`. `ACS-R1-05.1-T01` (Single Commercial Runtime Authority) cerro `accepted` (veredicto `single_commercial_runtime_authority_accepted`) - ver "Evidencia de cierre - ACS-R1-05.1-T01" en la release spec para el detalle completo. `ACS-R1-05.1-T02` (Stable Opportunity Continuity) cerro `accepted` (veredicto `stable_opportunity_continuity_accepted`) - la separacion cross-domain venta/postventa esta implementada y probada en el resolver pero todavia no demostrada en el camino nativo real de WhatsApp (`primaryIntent` sigue normalizando a `"unknown"` ahi) - ver "Evidencia de cierre - ACS-R1-05.1-T02" en la release spec para el detalle completo. Estos son campos separados, no un status compuesto: el lifecycle de la release (`parallel_in_progress`, ver "Release lifecycle" abajo) no cambia por ser el camino critico; `critical_path: true` es una bandera aparte que declara que este es el camino activo hacia el primer vertical conversacional operativo. `ACS-R1-04` no bloquea este camino: permanece `active_blocked_external` (unica release secuencial activa en el sentido de `AGENTS.md`) solo por Customer Service externo, y esa dependencia no pertenece hoy al camino critico del primer vertical conversacional por WhatsApp (aunque sigue siendo necesaria para identidad canonica, onboarding completo y operaciones transaccionales futuras).
