...
`ACS-R1-04-T05` debe incorporar Customer 360 al contexto autonomo, dentro del alcance ya definido por [customer-onboarding-identity-contract](../data/customer-onboarding-identity-contract.md) (seccion 10: Customer 360 solo se consume con un `customerId` valido, sin inyectar el snapshot completo cuando la operacion no lo necesita) y sin conectar todavia el `CustomerServicePort` de `ACS-R1-04-T04.1` al runtime (eso es `T06`).
