...
Ver contrato canonico: [customer-creation-linking-authority-contract.md](../data/customer-creation-linking-authority-contract.md). `ACS-R1-04-T04.1` debe implementar el Customer Service Port y las policies de `create_customer`, `link_external_identity` y `record_customer_interest` exactamente como estan definidas en ese contrato (inputs, outcomes, datos minimos, idempotencia, prevencion de duplicados, consentimiento y manejo de conflictos), sin inventar reglas nuevas ni reducir la autonomia estrategica de la IA descrita en su seccion 3.
