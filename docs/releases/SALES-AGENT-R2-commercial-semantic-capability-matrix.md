...
| Cancel shipping (scoped) | Drop shipping only, keep product | "no necesito despacho", "olvida el despacho", "cancela el despacho", "ya no quiero despacho", "sin despacho" | **Supported - live-validated 10/10 (100%) correct scope, A08.7** | Supported | `GET_SHIPPING_QUOTE` objective SUPERSEDED/CANCELLED, `SELECT_PRODUCTS`/`SET_DESTINATION` untouched | Supported | **Supported (A08.7 closure)** | A08.6's reported 0/8 was a benchmark-harness defect, not a planner gap - see A08.7 doc Part 1/18: the live planner already emitted the correctly-scoped intent (`{"scope":"shipping"}`) even before any prompt change; the setup turn just let the CommercialWork go terminal (COMPLETED) before the cancel turn arrived, so the cancel landed on a brand-new work with nothing carried forward - the benchmark's own before/after diff then misread "nothing left to preserve" as "everything cancelled". Fixed in the harness (keep the work non-terminal, exactly like the offline suite already did) and re-validated 10/10 |
| Cancel quote (scoped) | Drop the quote only, keep the rest | "no quiero cotizacion", "mejor no cotices", "olvida la cotizacion", "cancela la cotizacion", "no me hagas la cotizacion" | **Supported - live-validated 10/10 (100%) correct scope, A08.7** | Supported | `CREATE_QUOTE` objective SUPERSEDED/CANCELLED, rest untouched | Supported | **Supported (A08.7 closure)** | Same corrected root cause as cancel-shipping row |
| Cancel multiple scopes (same turn) | Drop 2+ named targets, keep the rest | "no quiero despacho ni cotizacion" | Supported (offline-verified, planner schema extended with `additionalScopes`) | Supported | Multiple cancel seeds in one turn, each independently scoped - engine already generalized to N seeds, no change needed | Supported | **Supported (offline-verified, A08.7)** | Not in the live minimum corpus (Part 18) - offline-verified only (CANCEL04, `commercialWorkSemanticCompleteness.test.ts`) |
| Cancel current objective (targeted, non-shipping/quote) | Drop one specific in-flight objective by name | (not independently sampled - blurs into the shipping/quote scoped-cancel rows above) | Unverified | N/A | Unverified | Unverified | **Unverified** | Not independently live-tested this session |
| Cancel whole work | Drop everything on this work | "olvidalo", "olvidalo todo", "dejalo", "no importa", "cancela eso" | Supported | Supported | Cancels every active objective | Supported | **Supported - live-validated 10/10 (100%), A08.6+A08.7** | None found |
