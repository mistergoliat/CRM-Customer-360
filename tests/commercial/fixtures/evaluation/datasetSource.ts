...
import type { CommercialContextCompleteness, CommercialContextWarning } from "../../../../lib/brain/commercial/constants";
import type {
...
import type { CommercialEvaluationDatasetMetadata, CommercialEvaluationInput, CommercialHumanReviewAssessment, CommercialProductiveDecisionObservation } from "../../../../lib/brain/commercial/evaluation";
import type { CommercialShadowExecutionDisposition, CommercialShadowMode, CommercialShadowStatus } from "../../../../lib/brain/commercial/shadow/shadowConstants";
import type { CommercialShadowWarning } from "../../../../lib/brain/commercial/shadow/shadowConstants";
import type { SalesAgentApprovalRequirement, SalesAgentConfidenceLevel, SalesAgentOutcome, SalesAgentRiskLevel, SalesAgentToolRequestStatus } from "../../../../lib/brain/commercial/sales-agent/validationTypes";
import type { SalesAgentRuntimeStatus } from "../../../../lib/brain/commercial/sales-agent/runtimeTypes";
import type { SalesAgentRequestedMode, SalesAgentToolName } from "../../../../lib/brain/commercial/salesAgentTypes";
...
type SyntheticProfile = {
  ...
  runtimeStatus: SalesAgentRuntimeStatus;
  validationStatus: "valid" | "invalid" | "failed_safe" | "skipped";
  policyStatus: CommercialPolicyStatus;
  overallDecision: CommercialPolicyDecision;
  outcome: SalesAgentOutcome;
  riskLevel: SalesAgentRiskLevel | CommercialPolicyRiskLevel;
  approvalRequirement: SalesAgentApprovalRequirement | CommercialPolicyApprovalRequirement;
  shouldRespondNow: boolean;
  confidence: SalesAgentConfidenceLevel;
  claimProfile: Array<SyntheticClaim>;
  blockedClaimProfile: Array<SyntheticClaim>;
  actionProfile: string[];
  blockedActionProfile: string[];
  toolProfile: string[];
  blockedToolProfile: string[];
  entityProfile: string[];
  blockedEntityProfile: string[];
  sourceSummary: {
  ...
  comparisonStatus: "aligned" | "partially_aligned" | "divergent" | "not_comparable";
  durationMs: number;
  ...
  model: string;
  warnings: Array<CommercialContextWarning | CommercialShadowWarning>;
  issueCodes: string[];
  appliedPolicyRules: string[];
  sideEffects?: Partial<CommercialShadowResult["sideEffects"]>;
  executionDisposition?: CommercialShadowExecutionDisposition;
};

type SyntheticProfileOverrides = Partial<Omit<SyntheticProfile, "sourceSummary" | "warnings" | "claimProfile" | "blockedClaimProfile" | "actionProfile" | "blockedActionProfile" | "toolProfile" | "blockedToolProfile" | "entityProfile" | "blockedEntityProfile" | "issueCodes" | "appliedPolicyRules">> & {
  sourceSummary?: Partial<SyntheticProfile["sourceSummary"]>;
  warnings?: Array<CommercialContextWarning | CommercialShadowWarning>;
  claimProfile?: SyntheticClaim[];
  blockedClaimProfile?: SyntheticClaim[];
  actionProfile?: string[];
  blockedActionProfile?: string[];
  toolProfile?: string[];
  blockedToolProfile?: string[];
...
const BASE_PROFILE: SyntheticProfile = {
...
function cloneProfile(overrides: SyntheticProfileOverrides = {}): SyntheticProfile {
  return {
    ...BASE_PROFILE,
    ...overrides,
    sourceSummary: {
    ...
    warnings: [...(overrides.warnings ?? BASE_PROFILE.warnings)] as CommercialShadowWarning[],
...
function toPolicyApprovalRequirement(value: SyntheticProfile["approvalRequirement"]): CommercialPolicyApprovalRequirement {
...
function toSalesAgentApprovalRequirement(value: SyntheticProfile["approvalRequirement"]): SalesAgentApprovalRequirement {
...
function buildClaims(profile: SyntheticProfile) {
...
function buildBlockedClaims(profile: SyntheticProfile) {
...
function buildCounts(values: readonly string[]) {
...
function buildCommercialContextSummary(profile: SyntheticProfile) {
  return {
    status: (profile.completeness === "insufficient" ? "insufficient_context" : "success") as "insufficient_context" | "success",
    completeness: profile.completeness,
    warnings: profile.warnings.filter((warning): warning is CommercialContextWarning => !warning.startsWith("shadow_")),
    sourceSummary: {
      ...
      latestOutboundAt: profile.sourceSummary.hasConversationHistory ? FIXED_TIME : null,
    ...
    metadata: {
      ...
      availableCapabilities: ["searchKnowledge"] as SalesAgentToolName[],
      recentMessagesLimit: 12,
      sanitized: true,
...
function buildRuntimeSummary(profile: SyntheticProfile) {
  return {
...
function buildPolicySummary(profile: SyntheticProfile) {
  return {
...
function buildGovernedResultSummary(profile: SyntheticProfile) {
  return {
    outcome: profile.outcome,
    confidence: profile.confidence,
    shouldRespondNow: profile.shouldRespondNow,
...
function buildMetrics(profile: SyntheticProfile) {
  return {
...
function buildShadowResult(profile: SyntheticProfile): CommercialShadowResult {
  const claims = buildClaims(profile);
  const blockedClaims = buildBlockedClaims(profile);
  const shadowWarnings = profile.warnings.filter((warning): warning is CommercialShadowWarning => warning.startsWith("shadow_"));
  ...
  return {
    ...
    mode: "shadow" as CommercialShadowMode,
    enabled: true,
    ...
    error:
      profile.runtimeStatus === "provider_error" || profile.runtimeStatus === "provider_unavailable"
        ? {
...
function buildSample(spec: SyntheticSampleSpec): CommercialEvaluationInput {
...
const sampleSpecs: SyntheticSampleSpec[] = [
  {
    sampleId: "sample-001",
...
function buildDatasetMetadata(): CommercialEvaluationDatasetMetadata {
  return {
...
export function buildCommercialEvaluationSyntheticDataset() {
  return {
...
export const COMMERCIAL_EVALUATION_SYNTHETIC_DATASET = buildCommercialEvaluationSyntheticDataset();
export { sampleSpecs as COMMERCIAL_EVALUATION_SYNTHETIC_SAMPLE_SPECS };
