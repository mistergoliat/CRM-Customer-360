/**
 * SALES-AGENT-R3-P7.11 (Confirmation Boundary Replication & Realistic Intent Validation). R0 (P7.10
 * S0, current semantics) vs R1 (P7.10 S1, consequence statement) through the SAME P7.8-R/P7.10 true
 * autonomous harness, on a NEW corpus (r3-p7-11.v1, 48 scenarios, not the P7.10 46). R3 does not
 * participate; S2 is not used.
 ...
 * BENCHMARK_LIVE_LLM_ENABLED=true AND --mode=live, and with the P7.8-R/P7.10 model overrides:
 ...
 *
 *   ... --mode=live --smoke --write-freeze=<file>            # smoke (10 runs), then record the freeze hashes
 *   ... --mode=live --runs=3 --freeze=<file>                 # full batch (288 runs; refuses to start if a frozen file changed)
 *   ... --mode=live --plan-only                              # prints the execution plan, runs nothing
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { fileURLToPath } from "node:url";
import { buildReplicationArtifactFiles, buildReplicationPlan, computeFreezeHashes, diffFreeze, interleavedScenarioIds, runReplication } from "../lib/brain/commercial/agent-loop/benchmark/r3ConfirmationBoundary/run";
import { classifierGoldenReport } from "../lib/brain/commercial/agent-loop/benchmark/r3MutationSemantics/runSemantics";
import { REPLICATION_VARIANT_IDS } from "../lib/brain/commercial/agent-loop/benchmark/r3ConfirmationBoundary/surfaces";
...
/** Smoke: 2 A (declarative), 2 B (multi-turn) and 1 E (informational negative) x R0/R1 = 10 runs. */
const SMOKE_CASE_IDS = ["P11A01", "P11A02", "P11B01", "P11B02", "P11E01"];
...
async function main() {
  try {
    ...
    if (args.planOnly) {
      const plan = buildReplicationPlan(caseIds, runsPerScenario);
      console.log(JSON.stringify({ runs: plan.length, first9: plan.slice(0, 9), variantsLeading: Object.fromEntries(REPLICATION_VARIANT_IDS.map((variant) => [variant, plan.filter((entry) => entry.positionInGroup === 0 && entry.variant === variant).length])) }, null, 2));
      return;
    }
    ...
    if (args.mode === "live") {
      ...
      if (wrong.length > 0) {
        console.error(`refusing to run: P7.8-R/P7.10 parity env missing/different: ${wrong.map(([key, value]) => `${key}=${value}`).join(" ")}`);
        process.exitCode = 1;
        return;
      }
    }
    ...
    if (golden.discrepancies > 0) {
      console.error(`refusing to run: the (P7.10, reused unchanged) confirmation classifier disagrees with ${golden.discrepancies} golden entries (0 required before the freeze).`);
      process.exitCode = 1;
      return;
    }
    if (args.freeze) {
    ...
    console.log(`P7.11 confirmation boundary replication - ${args.mode}, ${caseIds.length} scenario(s) x ${runsPerScenario} run(s) x ${REPLICATION_VARIANT_IDS.length} variants = ${caseIds.length * runsPerScenario * REPLICATION_VARIANT_IDS.length} runs.`);
    ...
    const runId = `p7-11-${String(result.bundle.manifest.startedAt).replace(/[:.]/g, "-")}-${args.mode}${args.smoke ? "-smoke" : args.cases ? "-partial" : ""}${args.label ? `-${args.label}` : ""}`;
    const runDir = join(ARTIFACT_ROOT, runId);
    mkdirSync(runDir, { recursive: true });
    for (const [fileName, content] of Object.entries(buildReplicationArtifactFiles(result.bundle))) writeFileSync(join(runDir, fileName), content, "utf8");

    console.log("");
    for (const variant of REPLICATION_VARIANT_IDS) {
      const m = result.bundle.summary[variant];
      console.log(`${variant.padEnd(28)} A-select=${fmt(m.byGroup.A.selectionRate)} A-unnecessaryConfirmation=${fmt(m.byGroup.A.unnecessaryConfirmationRate)} B-selectionAfterFacts=${fmt(m.multiTurn.selectionAfterFactsCompleteRate)} E-overMutation=${fmt(m.informational.informationalOverMutationRate)} harnessFailures=${m.harnessFailures}`);
    }
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  main().catch((error) => {
    console.error("P7.11 benchmark crashed unexpectedly:", error);
    process.exitCode = 1;
  });
}
