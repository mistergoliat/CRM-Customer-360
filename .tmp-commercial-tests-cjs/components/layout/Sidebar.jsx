...
function Sidebar() {
    ...
    return (<aside className="fixed inset-y-0 left-0 z-30 hidden w-sidebar-width flex-col bg-sidebar py-6 text-white lg:flex">
      <div className="px-6">
      ...
      <nav className="mt-8 flex-1 space-y-6 overflow-y-auto px-2">
        {groups.map((group) => ((() => {
            const visibleModules = modules_1.modules.filter((module) => module.group === group.key && module.navVisible !== false);
            ...
            return (<div key={group.key}>
                ...
                <div className="space-y-1">
                  {visibleModules.map((module) => {
                    ...
                    return (<link_1.default key={module.key} href={module.href} className={(0, clsx_1.default)("flex items-center gap-3 rounded-lg px-4 py-3 text-body-md transition", active
                            ? "bg-primary text-white shadow-lg"
                            : "text-tertiary-fixed-dim hover:bg-white/10 hover:text-white")}>
                        <Icon_1.Icon name={module.icon}/>
                        <span className="min-w-0 flex-1 font-bold">{module.label}</span>
                        {module.status !== "active" ? (<span className="rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-bold uppercase text-slate-200">
                            {(0, status_1.labelForModuleStatus)(module.status)}
                          </span>) : null}
                      </link_1.default>);
                })}
                </div>
