...
function containerClass(direction) {
    if (direction === "inbound")
        return "max-w-[88%] rounded-2xl rounded-bl-sm border border-slate-200 bg-white p-4";
    if (direction === "system")
        return "max-w-[88%] rounded-2xl border border-amber-100 bg-amber-50 p-4";
    return "ml-auto max-w-[88%] rounded-2xl rounded-br-sm border border-sky-100 bg-sky-50 p-4";
}
function CaseTimeline({ messages, source }) {
    if (messages.length === 0) {
        return (<div className="hub-card p-6">
    ...
    return (<div className="hub-card p-5">
      <div className="mb-5 flex items-center justify-between gap-3">
        ...
        <StatusChip_1.StatusChip label={`${messages.length} mensajes`} tone="gray"/>
      </div>
      <div className="space-y-4 bg-slate-50/70 p-2">
        {messages.map((message) => (<div key={message.key} className="grid gap-3 md:grid-cols-[140px_1fr]">
            <div className="pt-2 text-label-sm text-slate-500">{(0, format_1.formatDateTime)(message.occurredAt)}</div>
            <div className={containerClass(message.direction)}>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <StatusChip_1.StatusChip label={message.direction} tone={toneForDirection(message.direction)}/>
