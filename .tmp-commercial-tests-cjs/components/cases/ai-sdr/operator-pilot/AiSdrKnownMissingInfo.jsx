...
exports.AiSdrKnownMissingInfo = AiSdrKnownMissingInfo;
const react_1 = __importDefault(require("react"));
const CaseDetailPrimitives_1 = require("../../CaseDetailPrimitives");
function InfoCard({ title, items, emptyBody }) {
    return (<div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      {items.length === 0 ? (<div className="mt-3">
          ...
          {items.map((item, index) => (<div key={`${title}-${index}-${item.label}`} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
...
function AiSdrKnownMissingInfo({ pilot }) {
    ...
    return (<section className="grid gap-4">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <InfoCard title="Información conocida" items={pilot.knownInformation} emptyBody="No hay información conocida suficiente para el piloto operacional."/>
        <InfoCard title="Información faltante" items={pilot.missingInformation} emptyBody="No hay información faltante estructurada para este caso."/>
      </div>
    </section>);
}
