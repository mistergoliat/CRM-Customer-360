...
exports.ScenarioRunSummary = ScenarioRunSummary;
const StatusChip_1 = require("@/components/ui/StatusChip");
function toneForStatus(status) {
    const text = status.toLowerCase();
    if (text.includes("fail") || text.includes("invalid") || text.includes("blocked"))
        return "red";
    if (text.includes("pass") || text.includes("complete") || text.includes("deliver"))
        return "green";
    if (text.includes("partial") || text.includes("wait") || text.includes("retry"))
        return "amber";
    return "gray";
}
function ScenarioRunSummary({ result }) {
    if (!result) {
        return (<section className="rounded-[28px] border border-dashed border-slate-300 bg-white/70 p-4 text-slate-500">
    ...
    return (<section className="rounded-[28px] border border-slate-200 bg-white/95 p-4 shadow-[0_24px_90px_-45px_rgba(15,23,42,0.35)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <StatusChip_1.StatusChip label={result.status} tone={toneForStatus(result.status)}/>
      </div>
