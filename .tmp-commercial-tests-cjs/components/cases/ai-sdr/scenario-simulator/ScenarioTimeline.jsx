...
function ScenarioTimeline({ result }) {
    ...
    return (<section className="rounded-[28px] border border-slate-200 bg-white/95 p-4 shadow-[0_24px_90px_-45px_rgba(15,23,42,0.35)]">
      ...
      <ol className="mt-4 grid gap-4">
        {result.steps.map((step) => (<li key={step.stepId} className="rounded-[24px] border border-slate-200 bg-slate-50 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                <StatusChip_1.StatusChip label={step.loopResult.status}/>
                <StatusChip_1.StatusChip label={step.loopResult.finalStage} tone="slate"/>
                <StatusChip_1.StatusChip label={step.loopResult.outbox.workerResult?.status ?? "no outbox"} tone="gray"/>
              </div>
            </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              <div className="rounded-2xl bg-white p-3">
                ...
                <p className="mt-1 text-body-md text-on-surface">{step.loopResult.decision ? String(step.loopResult.decision.decision ?? step.loopResult.decision.type ?? "n/a") : "n/a"}</p>
              </div>
              <div className="rounded-2xl bg-white p-3">
                <p className="text-label-sm uppercase text-slate-500">Action</p>
                <p className="mt-1 text-body-md text-on-surface">{step.loopResult.action && typeof step.loopResult.action === "object" && "actionType" in step.loopResult.action ? String(step.loopResult.action.actionType ?? "n/a") : "n/a"}</p>
              </div>
              <div className="rounded-2xl bg-white p-3">
