...
exports.AiSdrSideEffects = AiSdrSideEffects;
const react_1 = __importDefault(require("react"));
const CaseDetailPrimitives_1 = require("../CaseDetailPrimitives");
const StatusChip_1 = require("@/components/ui/StatusChip");
function invariantRow(label, value) {
    return (<div className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2">
...
function AiSdrSideEffects({ review }) {
    ...
    return (<section className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <StatusChip_1.StatusChip label={invariant.violationDetected ? "critical" : "zero side effects"} tone={invariant.violationDetected ? "red" : "green"}/>
      </div>

      {invariant.violationDetected ? <CaseDetailPrimitives_1.CaseInlineNote tone="warning" title="Violación de invariantes" body="La observación contiene evidencia estructurada de un side effect o una contradicción que merece revisión crítica."/> : null}
      ...
      {invariant.violations.length > 0 ? (<div className="rounded-xl border border-rose-200 bg-rose-50 p-4">
          <p className="text-label-bold uppercase text-rose-800">Violations</p>
          <ul className="mt-3 space-y-2 text-body-md text-rose-900">
            {invariant.violations.map((violation) => (<li key={violation} className="rounded-lg bg-white px-3 py-2">
                {violation}
              </li>))}
          </ul>
        </div>) : null}
