...
exports.CaseReplyPanel = CaseReplyPanel;
...
function CaseReplyPanel({ caseId, closed, writeEnabled, whatsappWindowOpen, embedded = false }) {
    ...
    const content = (<>
      ...
      <textarea className="hub-textarea min-h-36 w-full resize-y" maxLength={4096} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Escribe una respuesta operacional..." disabled={!writeEnabled || closed || !whatsappWindowOpen || loading}/>
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        ...
        <button className="hub-button-primary shrink-0" disabled={!writeEnabled || closed || !whatsappWindowOpen || loading || message.trim().length === 0} title={disabledReason || "Enviar respuesta manual"} onClick={() => void submitReply()}>
    ...
    return (<CaseDetailPrimitives_1.CasePanelFrame title="Reply manual" description="Composer listo para Meta + trazabilidad DB, hoy bloqueado en modo read-only." accent="red">
      {content}
