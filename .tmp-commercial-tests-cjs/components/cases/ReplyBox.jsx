...
function ReplyBox({ caseId, closed, writeEnabled }) {
    ...
    return (<div className="hub-card p-5">
      ...
      <textarea className="hub-textarea min-h-32 w-full resize-y" maxLength={4096} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Escribe una respuesta operacional..." disabled={closed || actionBlocked}/>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <button className="hub-button-primary" title={actionBlocked ? actionBlockReason : "Enviar mensaje"} disabled={closed || actionBlocked || loading !== null || message.trim().length === 0} onClick={() => post(`/api/cases/${caseId}/reply`, { message_text: message })}>
          <Icon_1.Icon name="send"/>
          Enviar
        </button>
        <button className="hub-button-secondary" title={actionBlocked ? actionBlockReason : "Cerrar caso"} disabled={actionBlocked || loading !== null} onClick={() => post(`/api/cases/${caseId}/close`, {}, "Cerrar este caso?")}>
          <Icon_1.Icon name="task_alt"/>
          Cerrar
        </button>
        <button className="hub-button-secondary" title={actionBlocked ? actionBlockReason : "Reabrir caso"} disabled={actionBlocked || loading !== null} onClick={() => post(`/api/cases/${caseId}/reopen`)}>
          <Icon_1.Icon name="restart_alt"/>
          Reabrir
        </button>
        <button className="hub-button-secondary" title={actionBlocked ? actionBlockReason : "Bloquear IA"} disabled={actionBlocked || loading !== null} onClick={() => post(`/api/cases/${caseId}/block-ai`)}>
        ...
        <button className="hub-button-ghost" title={actionBlocked ? actionBlockReason : "Cambiar prioridad"} disabled={actionBlocked || loading !== null} onClick={() => post(`/api/cases/${caseId}/priority`, { priority })}>
          Cambiar prioridad
        </button>
      </div>
