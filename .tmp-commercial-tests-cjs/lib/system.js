...
async function getSystemHealth() {
    const items = [];
    const db = await (0, db_1.safeScalar)("SELECT 1");
    items.push({
        key: "db",
        title: "Base de datos",
    ...
    const openCases = await (0, db_1.safeScalar)("SELECT COUNT(*) FROM n8n_vw_hub_cases WHERE COALESCE(status, '') NOT IN ('closed', 'resolved', 'done')");
    items.push({
        key: "open_cases",
    ...
    const inbound = await (0, cases_1.recentInboundMessages)(5);
    items.push({
        key: "inbound",
    ...
    const outbound = await (0, cases_1.recentOutboundMessages)(5);
    items.push({
        key: "outbound",
    ...
    items.push({
        ...
        description: process.env.META_ACCESS_TOKEN && (process.env.DEFAULT_PHONE_NUMBER_ID || process.env.META_PHONE_NUMBER_ID)
    ...
    items.push({
        key: "db_write",
        title: "DB writer",
        status: (0, write_access_1.isDbWriteEnabled)() ? "ok" : "warning",
        description: (0, write_access_1.isDbWriteEnabled)()
