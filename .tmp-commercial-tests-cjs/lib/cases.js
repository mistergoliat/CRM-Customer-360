...
exports.getCaseFilterOptions = getCaseFilterOptions;
exports.updateCaseLifecycle = updateCaseLifecycle;
exports.insertManualOutbound = insertManualOutbound;
exports.recentInboundMessages = recentInboundMessages;
exports.recentOutboundMessages = recentOutboundMessages;
exports.getCaseTableColumns = getCaseTableColumns;
...
async function listCases(filters) {
    ...
    const countResult = await (0, db_1.safeQueryRows)(`SELECT COUNT(*) AS total FROM ${CASES_VIEW} ${whereSql}`, params);
    const rowsResult = await (0, db_1.safeQueryRows)(`SELECT * FROM ${CASES_VIEW} ${whereSql} ORDER BY COALESCE(last_message_at, updated_at, created_at) DESC LIMIT ${pageSize} OFFSET ${offset}`, params);
    if (!rowsResult.ok) {
...
async function getCaseById(id) {
    const rows = await (0, db_1.safeQueryRows)(`SELECT * FROM ${CASES_VIEW} WHERE conversation_case_id = ? LIMIT 1`, [id]);
    if (!rows.ok)
        return { ok: false, error: rows.error, row: null };
...
function normalizeDirection(value, row, source) {
    ...
    if (normalized === "manual")
        return "manual";
    if (source === WA_MESSAGES_TABLE)
        return "inbound";
    const sourceTable = String(row.source_table || "").toLowerCase();
    const finalAction = String(row.final_action || "").toLowerCase();
    if (sourceTable.includes("hub") || finalAction.includes("manual"))
        return "manual";
    if (sourceTable.includes("system") || finalAction.includes("notify"))
        return "system";
    if (sourceTable.includes("outbound"))
        return "outbound";
    return "system";
}
function normalizeTimelineRows(rows, source) {
    return rows
        .map((row, index) => {
...
async function getCaseTimeline(caseRow) {
    const caseId = caseRow.conversation_case_id;
    const messageColumns = await (0, db_1.getColumns)(MESSAGES_TABLE);
    ...
    if (messageColumns.includes("conversation_case_id")) {
        ...
        const rows = await (0, db_1.safeQueryRows)(`SELECT * FROM ${MESSAGES_TABLE} WHERE conversation_case_id = ?${statusFilter} ORDER BY ${dateColumn ? `\`${dateColumn}\`` : "1"} ASC`, [caseId]);
        if (rows.ok && rows.rows.length > 0) {
            return { ok: true, rows: normalizeTimelineRows(rows.rows, MESSAGES_TABLE), source: MESSAGES_TABLE };
        }
    }
    const waId = caseRow.wa_id;
    const fallbackColumns = await (0, db_1.getColumns)(WA_MESSAGES_TABLE);
    ...
    if (waId && fallbackColumns.includes("wa_id")) {
        const rows = await (0, db_1.safeQueryRows)(`SELECT * FROM ${WA_MESSAGES_TABLE} WHERE wa_id = ? ORDER BY ${fallbackDateColumn ? `\`${fallbackDateColumn}\`` : "1"} ASC LIMIT 200`, [waId]);
        if (rows.ok)
            return { ok: true, rows: normalizeTimelineRows(rows.rows, WA_MESSAGES_TABLE), source: WA_MESSAGES_TABLE };
        return { ok: false, rows: [], source: WA_MESSAGES_TABLE, error: rows.error };
    }
    return { ok: true, rows: [], source: "empty" };
}
async function getCaseFilterOptions(column) {
    const allowed = new Set(["status", "department", "priority"]);
    if (!allowed.has(column))
        return [];
    const result = await (0, db_1.safeQueryRows)(`SELECT DISTINCT ${column} AS value FROM ${CASES_VIEW} WHERE ${column} IS NOT NULL AND ${column} <> '' ORDER BY ${column} ASC LIMIT 50`);
    return result.ok ? result.rows.map((row) => row.value).filter(Boolean) : [];
}
async function updateCaseLifecycle(id, values) {
    return (0, db_1.updateExistingColumns)(CASES_TABLE, ["conversation_case_id", "id"], id, values);
}
async function tryInsert(tableName, values, requiredAny) {
    try {
        return await (0, db_1.insertExistingColumns)(tableName, values, requiredAny);
    }
    catch (error) {
        return { ok: false, warning: error instanceof Error ? error.message : String(error) };
    }
}
async function insertManualOutbound(caseRow, messageText, providerMessageId) {
    const baseValues = {
...
async function recentInboundMessages(limit = 20) {
    const columns = await (0, db_1.getColumns)(WA_MESSAGES_TABLE);
    if (columns.length === 0)
        return { ok: true, rows: [] };
    ...
    return (0, db_1.safeQueryRows)(`SELECT * FROM ${WA_MESSAGES_TABLE} ${where} ORDER BY ${dateColumn ? `\`${dateColumn}\`` : "1"} DESC LIMIT ${limit}`);
}
async function recentOutboundMessages(limit = 20) {
    const columns = await (0, db_1.getColumns)(MESSAGES_TABLE);
    if (columns.length === 0)
        return { ok: true, rows: [] };
    ...
    return (0, db_1.safeQueryRows)(`SELECT * FROM ${MESSAGES_TABLE} ${where} ORDER BY ${dateColumn ? `\`${dateColumn}\`` : "1"} DESC LIMIT ${limit}`);
}
async function getCaseTableColumns() {
    return (0, db_1.getColumns)(CASES_TABLE);
}
