...
exports.resolveBrainResponsePolicy = resolveBrainResponsePolicy;
...
function isEmptyMessage(messageText) {
...
function buildBlockedReasons(request) {
    const reasons = [];
    const signals = request.botEligibility?.signals;
    if (request.options.executeActions)
        reasons.push("execute_actions_disabled");
    if (signals?.suppression_active)
        reasons.push("suppression_active");
    if (signals?.active_human_case)
        reasons.push("active_human_case");
    if (signals?.recent_manual_reply)
        reasons.push("recent_manual_reply");
    if (signals?.closed_or_rejected_case)
        reasons.push("closed_or_rejected_case");
    if (signals?.manual_operator_lock)
        reasons.push("manual_operator_lock");
    if (request.contextSummary.partialContext)
        reasons.push("partial_context");
    return reasons;
}
function buildDecision(request, blockedReasons) {
    if (request.options.executeActions)
        return "blocked";
    const cleanMessage = normalizeText(request.messageText);
    if (isEmptyMessage(request.messageText))
        return "no_action";
    const signals = request.botEligibility?.signals;
    if (signals?.suppression_active)
        return "blocked";
...
function resolveBrainResponsePolicy(request) {
    ...
    return {
        ...
        confidence,
        signals,
        suggested_next_step: decision === "blocked"
