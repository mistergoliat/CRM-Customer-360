...
exports.buildBrainInstructions = buildBrainInstructions;
const instructions_1 = require("../instructions");
function makeInstruction(id, kind, status, target, enabled, reason, payload) {
    return {
        id,
        kind,
        status,
...
function buildBrainInstructions(request, context, state, enrichment) {
    ...
    const actionPolicy = enrichment?.actionPolicy ??
        {
    ...
    const normalizedAction = enrichment?.normalizedAction ??
        {
            action: "context_only",
    ...
    const steps = [
        makeInstruction("continue-legacy-flow", "continue_legacy_flow", continueLegacyFlow ? "planned" : "blocked", "n8n", continueLegacyFlow, continueLegacyFlow
        ...
        makeInstruction("record-observation", "record_observation", "planned", "n8n", canReturnInstructions, canReturnInstructions
        ...
        makeInstruction("resolve-action-policy", "noop", "planned", "backend", true, "Resolve deterministic response policy and router state without side effects.", {
            policyId: actionPolicy.policyId,
            decision: actionPolicy.decision,
            blockedReasons,
            suggestedNextStep: actionPolicy.suggested_next_step
        }),
        makeInstruction("shadow-ai-orchestrator", "shadow_ai_orchestrator_call", canSuggestShadowCall ? "planned" : "blocked", "n8n", canSuggestShadowCall, canSuggestShadowCall
    ...
    return {
        version: instructions_1.BRAIN_INSTRUCTIONS_VERSION,
