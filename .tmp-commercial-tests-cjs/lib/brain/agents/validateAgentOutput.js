...
exports.validateBrainAgentOutput = validateBrainAgentOutput;
...
function validateBrainAgentOutput(definition, output) {
    const errors = [];
    if (!isRecord(output)) {
    ...
    if (!Array.isArray(output.toolRequests)) {
        ...
        for (const toolRequest of output.toolRequests) {
            ...
            if (!types_1.BRAIN_TOOL_NAMES.includes(toolRequest.toolName)) {
