...
exports.normalizeBrainAgentRunRequest = normalizeBrainAgentRunRequest;
exports.runAgent = runAgent;
const responsePolicy_1 = require("../actions/responsePolicy");
const instructions_1 = require("../instructions");
const registry_1 = require("./registry");
const agentRunLog_1 = require("./agentRunLog");
const validateAgentOutput_1 = require("./validateAgentOutput");
...
function normalizeContextPack(input) {
    if (!isRecord(input))
        return null;
    return {
        ...
        related_case_id: input.related_case_id === undefined ? null : input.related_case_id,
        related_order_id: input.related_order_id === undefined ? null : input.related_order_id
    };
}
function normalizeInputEvent(input) {
    ...
    const errors = [];
    if (!channel)
        errors.push(error("inputEvent.channel must be whatsapp."));
    if (!source)
        errors.push(error("inputEvent.source is required."));
    if (!waId)
        errors.push(error("inputEvent.wa_id is required."));
    if (!phoneNumberId)
        errors.push(error("inputEvent.phone_number_id is required."));
    if (!messageId)
        errors.push(error("inputEvent.message_id is required."));
    if (!messageText)
        errors.push(error("inputEvent.message_text is required."));
    if (errors.length > 0)
    ...
    return {
        ...
        value: {
            ...
            conversation_case_id: input.conversation_case_id,
...
function normalizeContext(input) {
    ...
    if (!status)
        errors.push(error("context.status is required."));
    if (!source)
        errors.push(error("context.source is required."));
    if (!traceId)
        errors.push(error("context.traceId is required."));
    if (!waId)
        errors.push(error("context.waId is required."));
    if (!phoneNumberId)
        errors.push(error("context.phoneNumberId is required."));
    if (!messageId)
        errors.push(error("context.messageId is required."));
    if (errors.length > 0)
        return { ok: false, value: null, errors };
    return {
        ok: true,
        value: {
            ...
            customerRef: isRecord(input.customerRef) ? input.customerRef : undefined,
...
function normalizeActionPolicy(input) {
    ...
    const errors = [];
    if (!allowedDecision)
        errors.push(error("actionPolicy.decision is invalid."));
    if (!asString(input.policyId))
        errors.push(error("actionPolicy.policyId is required."));
    if (!asString(input.reason))
        errors.push(error("actionPolicy.reason is required."));
    if (errors.length > 0)
        return { ok: false, value: null, errors };
    return {
        ok: true,
        value: {
            policyId: asString(input.policyId),
            decision: decision,
            ...
            confidence: asNumber(input.confidence, 0),
            signals: Array.isArray(input.signals) ? input.signals.filter((item) => typeof item === "string") : [],
            suggested_next_step: asString(input.suggested_next_step) === "legacy_continue" ||
                ...
                ? asString(input.suggested_next_step)
                : "context_only"
        },
        errors: []
    };
}
function normalizeContextPacks(input) {
    if (!isRecord(input))
        return {};
    return {
...
function normalizeBrainAgentRunRequest(input) {
    if (!isRecord(input)) {
    ...
    if (!agentName)
        errors.push(error("agentName is required."));
    if (!inputEventResult.ok)
        errors.push(...inputEventResult.errors);
    if (!contextResult.ok)
        errors.push(...contextResult.errors);
    if (!actionPolicyResult.ok)
        errors.push(...actionPolicyResult.errors);
    if (options.executeActions)
        errors.push(error("executeActions=true is not allowed for agent runs.", { code: "ACTION_BLOCKED" }));
    const validAgentName = agentName === "knowledge" ||
        agentName === "sales" ||
        agentName === "sac" ||
        agentName === "postventa" ||
        agentName === "campaign" ||
        ...
        : null;
    if (!validAgentName)
        errors.push(error("agentName is not registered."));
    if (errors.length > 0) {
        return { ok: false, value: null, errors };
    }
    return {
        ...
        value: {
            ...
            inputEvent: inputEventResult.value,
            context: contextResult.value,
            contextPacks: normalizeContextPacks(input.contextPacks),
            actionPolicy: actionPolicyResult.value,
            options,
...
function buildBlockedResponse(request, definition, startedAt, reason, validationErrors, warnings = []) {
    const requestId = request.requestId ?? request.context.traceId ?? (0, instructions_1.makeBrainTraceId)({ source: request.context.source, messageId: request.context.messageId, waId: request.context.waId });
    return {
        ok: false,
        requestId,
...
function filterToolRequests(definition, toolRequests) {
...
async function runAgent(input, startedAt = Date.now()) {
    const normalizedResult = normalizeBrainAgentRunRequest(input);
    if (!normalizedResult.ok) {
        const fallbackDefinition = (0, registry_1.getBrainAgentDefinition)("knowledge");
        return buildBlockedResponse({
            ...
            contextPacks: {},
            actionPolicy: (0, responsePolicy_1.resolveBrainResponsePolicy)({
            ...
            requestId: `brain-agent-${Date.now()}`
        }, fallbackDefinition, startedAt, normalizedResult.errors[0]?.message ?? "Invalid agent request.", normalizedResult.errors);
    }
    const request = normalizedResult.value;
    const definition = (0, registry_1.getBrainAgentDefinition)(request.agentName);
    if (!definition) {
        return buildBlockedResponse(request, (0, registry_1.getBrainAgentDefinition)("knowledge"), startedAt, "Agent is not registered.", [error("Agent is not registered.")]);
    }
    if (!definition.enabled || !(0, registry_1.isBrainAgentRunnable)(definition.name)) {
        return buildBlockedResponse(request, definition, startedAt, `Agent ${definition.name} is not enabled for runtime execution yet.`, [error(`Agent ${definition.name} is not enabled for runtime execution yet.`)], ["agent_disabled"]);
    }
    ...
    if (request.actionPolicy.decision === "blocked") {
        return buildBlockedResponse(request, definition, startedAt, request.actionPolicy.reason, [], [...contextWarnings, ...request.actionPolicy.blocked_reasons.map((reason) => `Policy blocked: ${reason}`)]);
    }
    if (request.agentName === "knowledge" && !request.options.dryRun) {
        return buildBlockedResponse(request, definition, startedAt, "Knowledge Agent is only runnable in dry-run mode.", [error("Knowledge Agent is only runnable in dry-run mode.")], ["dry_run_required"]);
    }
    const modelResult = await (0, modelAdapter_1.runBrainModelAdapter)({
        agentName: definition.name,
        agentVersion: definition.version,
    ...
    const outputValidation = (0, validateAgentOutput_1.validateBrainAgentOutput)(definition, modelResult.output);
    if (!outputValidation.ok) {
        return buildBlockedResponse(request, definition, startedAt, "Model adapter returned invalid output.", outputValidation.errors, [...contextWarnings, ...modelResult.warnings, ...modelResult.safetyFlags]);
    }
    ...
    const logResult = await (0, agentRunLog_1.recordBrainAgentRun)(response);
