...
exports.validateKnowledgeAgentOutput = validateKnowledgeAgentOutput;
...
function containsForbiddenOperationalClaims(text) {
    const normalized = text.toLowerCase();
    return FORBIDDEN_OPERATIONAL_TERMS.some((term) => normalized.includes(term));
}
function validateKnowledgeAgentOutput(output) {
    const errors = [];
    if (!output || typeof output !== "object" || Array.isArray(output)) {
        return { ok: false, value: null, errors: [error("Knowledge agent output must be an object.")] };
    }
    const draft = output;
    if (!isDecision(draft.decision))
        errors.push(error("decision is invalid."));
    if (!draft.answer_type)
        errors.push(error("answer_type is required."));
    ...
    return errors.length > 0 ? { ok: false, value: null, errors } : { ok: true, value: draft, errors: [] };
}
