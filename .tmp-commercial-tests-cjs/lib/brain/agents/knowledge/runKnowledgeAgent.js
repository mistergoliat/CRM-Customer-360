...
function mapDecisionToAnswerType(decision, query) {
    if (decision === "route_to_sales")
        return "generic";
    if (decision === "route_to_sac")
        return "generic";
    if (decision === "route_to_postventa")
        return "generic";
    if (isBusinessInfoRequest(query)) {
        if (hasAny(query, ["horario", "atencion", "atención"]))
            return "business_info";
        if (hasAny(query, ["ubicacion", "ubicación", "retiro"]))
            return "location";
        if (hasAny(query, ["pago", "medios de pago"]))
            return "payment";
...
function buildToolRequests(query) {
    return [
        {
            toolName: "getKnowledgePolicy",
            status: "planned",
        ...
        {
            toolName: "getStaticBusinessInfo",
            status: "planned",
...
function createAbstainDraft(request, reason, warnings) {
...
function buildMockDraft(request) {
    const query = request.inputEvent.message_text;
    const staticBusinessInfo = (0, knowledge_1.getStaticBusinessInfo)();
    const knowledgeHit = (0, knowledge_1.searchKnowledge)(query);
    const warnings = [];
...
async function callRealKnowledgeModel(request) {
    ...
    if (!endpoint || !apiKey)
        return null;
    const prompt = (0, prompt_1.buildKnowledgeAgentPrompt)(request);
    ...
    try {
                ...
                messages: [
                    { role: "system", content: prompt.system },
                    {
                        ...
                        content: JSON.stringify({
                            ...
                            policy: (0, knowledge_1.getKnowledgePolicy)(),
                            businessInfo: (0, knowledge_1.getStaticBusinessInfo)()
                        })
                    }
                ]
            })
        ...
        return {
            ...
            sources_used: Array.isArray(parsed.sources_used) ? parsed.sources_used.filter((item) => typeof item === "string") : ["knowledge_policy"],
            safety_flags: Array.isArray(parsed.safety_flags) ? parsed.safety_flags.filter((item) => typeof item === "string") : ["real_model"],
...
function buildResponse(request, output, startedAt, runtimeMode) {
    const requestId = request.requestId ?? request.context.traceId ?? (0, instructions_1.makeBrainTraceId)({
...
function buildFailedResponse(request, reason, startedAt, warnings = []) {
    const requestId = request.requestId ?? request.context.traceId ?? (0, instructions_1.makeBrainTraceId)({
        source: request.context.source,
        messageId: request.context.messageId,
        waId: request.context.waId
...
async function runKnowledgeAgent(request, startedAt = Date.now()) {
    ...
    const validation = (0, validate_1.validateKnowledgeAgentOutput)(output);
    if (!validation.ok) {
