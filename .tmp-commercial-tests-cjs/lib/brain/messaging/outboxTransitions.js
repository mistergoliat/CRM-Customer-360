...
exports.transitionOutboxStatus = transitionOutboxStatus;
...
function buildBaseResult(outboxId, dedupeKey, fromStatus, toStatus, simulated, applied, reason, blockedReasons = [], warnings = [], metadata) {
    return {
        outbox_id: outboxId,
...
function transitionOutboxStatus(input) {
    ...
    return buildBaseResult(input.outboxId ?? null, input.dedupeKey, input.fromStatus, input.toStatus, simulated, applied, reason, input.toStatus === "blocked" ? ["blocked_by_policy"] : [], transitionWarnings, {
        ...input.metadata,
        locked_at: input.lockedAt ?? null,
        failed_at: input.failedAt ?? null
    });
