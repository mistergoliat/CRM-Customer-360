...
exports.BRAIN_PERSIST_CANONICAL_OUTBOUND_FLAG = exports.BRAIN_CANONICAL_OUTBOUND_TABLE = void 0;
exports.persistCanonicalOutboundMessage = persistCanonicalOutboundMessage;
const db_1 = require("@/lib/db");
const outbox_1 = require("./outbox");
exports.BRAIN_CANONICAL_OUTBOUND_TABLE = "n8n_conversation_messages";
exports.BRAIN_PERSIST_CANONICAL_OUTBOUND_FLAG = "BRAIN_PERSIST_CANONICAL_OUTBOUND";
...
function buildPersistResult(status, messageId, warnings = []) {
...
function buildLookupClauses(columns, input) {
    const clauses = [];
    const params = [];
    const providerMessageId = asTrimmedString(input.providerMessageId);
    const dedupeKey = asTrimmedString(input.dedupeKey);
    let finalSourceRequestId = asTrimmedString(input.sourceRequestId);
    if (!finalSourceRequestId)
        finalSourceRequestId = dedupeKey;
    if (!finalSourceRequestId)
        finalSourceRequestId = asTrimmedString(input.providerMessageId);
    if (!finalSourceRequestId && input.outboxId !== null)
        finalSourceRequestId = `brain_outbox:${input.outboxId}`;
    ...
    if (input.outboxId !== null) {
        if (columns.includes("source_table") && columns.includes("source_id")) {
            ...
            params.push(outbox_1.BRAIN_MESSAGE_OUTBOX_TABLE, input.outboxId);
        }
...
function buildSchemaAdaptationWarnings(columns, input) {
    const warnings = [];
    const providerMessageId = asTrimmedString(input.providerMessageId);
    const dedupeKey = asTrimmedString(input.dedupeKey);
...
function resolveCanonicalDedupeKey(input) {
    const candidates = [
...
function buildInsertValues(input) {
    ...
    return {
        ...
        source: "brain",
        source_table: outbox_1.BRAIN_MESSAGE_OUTBOX_TABLE,
...
function buildInsertSql(columns, values) {
    const insertColumns = [];
    const placeholders = [];
    const params = [];
    for (const [column, value] of Object.entries(values)) {
        ...
        if (value === "__CHILE_NOW__") {
            placeholders.push((0, db_1.chileNowSql)());
            continue;
        }
        placeholders.push("?");
        params.push(value);
...
async function lookupCanonicalOutboundMessage(connection, clauses, params) {
    const sql = `SELECT * FROM \`${exports.BRAIN_CANONICAL_OUTBOUND_TABLE}\`${clauses.length > 0 ? ` WHERE ${clauses.join(" OR ")}` : ""} LIMIT 1`;
...
async function persistCanonicalOutboundMessage(input) {
    ...
    const columns = await (0, db_1.getColumns)(exports.BRAIN_CANONICAL_OUTBOUND_TABLE);
    if (columns.length === 0) {
        return buildPersistResult("skipped", null, [`Tabla ${exports.BRAIN_CANONICAL_OUTBOUND_TABLE} no disponible`]);
    }
    ...
    if (!columns.includes("message_text") && !columns.includes("text") && !columns.includes("body") && !columns.includes("message") && !columns.includes("content") && !columns.includes("raw_text")) {
        return buildPersistResult("skipped", null, [
            `Tabla ${exports.BRAIN_CANONICAL_OUTBOUND_TABLE} no contiene columnas compatibles para el texto del mensaje.`
        ]);
    }
    ...
    if (insert.insertColumns.length === 0) {
        return buildPersistResult("skipped", null, [
            `Sin columnas insertables en ${exports.BRAIN_CANONICAL_OUTBOUND_TABLE}.`
        ]);
    }
    warnings.push(...buildSchemaAdaptationWarnings(columns, input));
    const sql = `INSERT IGNORE INTO \`${exports.BRAIN_CANONICAL_OUTBOUND_TABLE}\` (${insert.insertColumns.join(", ")}) VALUES (${insert.placeholders.join(", ")})`;
    try {
        return await (0, db_1.withConnection)(async (connection) => {
            ...
            const [insertResult] = await connection.execute(sql, insert.params);
            ...
            return buildPersistResult("warning", null, [
                `Canonical outbound row insert completed but ${exports.BRAIN_CANONICAL_OUTBOUND_TABLE} lookup could not confirm the row.`
            ]);
        });
    }
    catch (error) {
