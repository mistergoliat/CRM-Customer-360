...
const buildInstructions_1 = require("./actions/buildInstructions");
const actionRouter_1 = require("./actions/actionRouter");
const runAgent_1 = require("./agents/runAgent");
const resolveContext_1 = require("./context/resolveContext");
const legacyAdapters_1 = require("./context/legacyAdapters");
const normalize_1 = require("./inbound/normalize");
const responseExecutor_1 = require("./messaging/responseExecutor");
const outbox_1 = require("./messaging/outbox");
const instructions_1 = require("./instructions");
const policy_1 = require("./commercial/policy");
const shadow_1 = require("./commercial/shadow");
const createCommercialShadowFailedSafe_1 = require("./commercial/shadow/createCommercialShadowFailedSafe");
const runCommercialShadowEvaluation_1 = require("./commercial/shadow/runCommercialShadowEvaluation");
const evaluation_1 = require("./commercial/evaluation");
const operational_loop_1 = require("./commercial/operational-loop");
const policy_2 = require("./commercial/policy");
const runtimeTypes_1 = require("./commercial/sales-agent/runtimeTypes");
...
function mergeBrainErrors(...groups) {
...
function getProcessInboundPersistOutboxPlanRequested(input) {
    if (!isRecord(input))
        return false;
    const options = isRecord(input.options) ? input.options : null;
    return asLooseBoolean(options?.persistOutboxPlan);
}
function getProcessInboundOutboxPlanEnabled() {
    return process.env.BRAIN_PROCESS_INBOUND_ALLOW_OUTBOX_PLAN === "true";
}
function canAutoReplyForInboundOutboxPlan(actionPolicy) {
    const policy = actionPolicy;
    return Boolean(policy.allowedToAutoReply ?? actionPolicy.can_auto_reply);
}
function mapInboundSourceToExecutionSource(source) {
    if (source === "n8n_meta_webhook")
        return "n8n";
    if (source === "hub_preview")
        return "operator";
    if (source === "manual_test")
        return "operator";
    return "brain";
}
function buildSkippedOutboxPlanResult(status, reason) {
    return {
        status,
        existing: false,
        outbox_id: null,
...
function buildWarningOutboxPlanResult(reason, warning) {
    return {
        status: "warning",
        existing: false,
        outbox_id: null,
...
function buildPersistedOutboxPlanResult(persistResult) {
    if (persistResult.ok) {
        return {
...
function buildFallbackPolicy(requestId, reason, blockedReasons) {
    return {
...
function buildFallbackNormalizedAction(reason, blockedReasons) {
    return {
        action: "blocked",
...
function buildContextSummary(requestId, request, contextResponse) {
...
function buildFailedContextSummary(request, requestId, warnings) {
    return {
        requestId,
        partialContext: true,
        waId: request.waId,
...
function buildBrainContextDebugPayload(request, contextResponse, context) {
    return {
        input: {
...
function toExecutionActionPolicy(actionPolicy) {
    return {
...
function toExecutionBotEligibility(botEligibility, contextSummary) {
    if (!botEligibility)
        return undefined;
...
function shouldBuildExecutionPlan(request, actionResponse, agentDraft, contextResponse, contextSummary) {
    if (!request.options.buildExecutionPlanDryRun)
        return false;
    if (request.options.executeActions)
        return false;
    if (request.options.dryRun !== true)
        return false;
    if (!agentDraft || agentDraft.decision !== "answer")
        return false;
    if (!actionResponse.action_policy.can_auto_reply)
        return false;
    if (contextResponse.bot_eligibility && !contextResponse.bot_eligibility.can_auto_reply)
        return false;
    if (contextSummary.warnings.some((warning) => warning.toLowerCase().includes("invalid")))
        return false;
    return true;
}
function buildExecutionPlanPreview(request, actionResponse, agentDraft, contextSummary, contextResponse) {
    ...
    return (0, responseExecutor_1.evaluateBrainExecution)(executionRequest);
}
function buildFailClosedResponse(request, normalized, startedAt, errors, reason, blockedReasons) {
    const requestId = (0, instructions_1.makeBrainRequestId)(request);
    const context = (0, resolveContext_1.resolveBrainContext)(request);
    ...
    const instructions = (0, buildInstructions_1.buildBrainInstructions)(request, context, { valid: false, failClosedReason: reason }, {
        contextSummary,
        botEligibility: null,
        contextPacksAvailable: [],
...
function buildActionRequest(request, requestId, contextSummary, contextResponse) {
    return {
        requestId,
        source: request.source,
        waId: request.waId,
...
function buildAgentRequest(request, requestId, contextResponse, actionResponse) {
    return {
        agentName: "knowledge",
        requestId,
        inputEvent: {
...
function shouldRunKnowledgeAgent(request, actionResponse, contextResponse) {
    ...
    if (!request.options.runAgentDryRun)
        return false;
    if (request.options.preferredAgent && request.options.preferredAgent !== "knowledge")
        return false;
    const normalizedAction = actionResponse.normalized_action.action;
    if (normalizedAction === "blocked" || normalizedAction === "no_action")
        return false;
    ...
    const hasCriticalBlocker = Boolean(botEligibility) &&
        botEligibility !== null &&
    ...
    if (hasCriticalBlocker)
        return false;
    if (suggestedNextStep === "context_only")
        return true;
    if (serviceContext.primary_service === "knowledge")
        return true;
    return request.options.preferredAgent === "knowledge";
}
async function maybePersistInboundOutboxPlan(request, actionResponse, agentDraft, contextResponse, persistOutboxPlanRequested) {
    ...
    const persistResult = await (0, outbox_1.createOutboxPlannedRecord)({
        dedupeKeyInput: {
...
function buildValidResponse(request, startedAt, contextResponse, actionResponse, agentDraft, executionPlan, outboxPlanResult, outboxPlanWarnings = [], commercialShadowResult = null, commercialOperationalLoopResult = null) {
    const requestId = (0, instructions_1.makeBrainRequestId)(request);
    const context = (0, resolveContext_1.resolveBrainContext)(request);
    ...
    const safeInstructions = {
        ...
        steps: actionResponse.instructions.steps
    };
    const warnings = mergeUniqueStrings(contextSummary.warnings, contextResponse.warnings, actionResponse.warnings, outboxPlanWarnings, commercialOperationalLoopResult?.warnings ?? [], request.options.debug ? [] : ["Full context payload is hidden unless debug=true."]);
...
function buildCommercialShadowFlags(input) {
    return {
        ...shadow_1.COMMERCIAL_SHADOW_DEFAULT_FEATURE_FLAGS,
        ...(input?.commercialShadowFlags ?? {})
    };
}
function buildCommercialOperationalLoopFlags(input) {
    return {
...
function buildCommercialOperationalLoopInput(request, contextResponse, commercialShadowResult, startedAt, correlationId, featureFlags) {
    if (!commercialShadowResult)
        return null;
    const commercialEvaluationResult = (0, evaluation_1.evaluateCommercialShadowResult)({
...
function buildCommercialShadowInput(request, contextResponse, startedAt, flags) {
    return {
        inboundMessage: request,
        brainContext: contextResponse,
        correlationId: (0, instructions_1.makeBrainRequestId)(request),
        executionId: null,
        currentTime: new Date(startedAt).toISOString(),
        timezone: "UTC",
        ...
        runtimeOptions: {
            enabled: true,
            mode: "dry_run",
            timeoutMs: shadow_1.COMMERCIAL_SHADOW_RUNTIME_TIMEOUT_MS,
        ...
        policyVersion: policy_2.COMMERCIAL_POLICY_VERSION,
        allowedCapabilities: [],
        metadata: request.metadata
    };
}
function sanitizeCommercialShadowErrorMessage(message) {
    return message
...
async function processInbound(input, startedAt = Date.now(), dependencies = {}) {
    const normalizedResult = (0, normalize_1.normalizeBrainInboundRequest)(input);
    if (!normalizedResult.ok) {
        const fallbackRequest = (0, normalize_1.buildFallbackBrainInboundRequest)(input);
        return buildFailClosedResponse(fallbackRequest, null, startedAt, normalizedResult.errors, normalizedResult.errors[0]?.message ?? "Invalid inbound request.", ["invalid_input"]);
    }
    const request = normalizedResult.value;
    const resolveContext = dependencies.resolveBackendBrainContext ?? resolveContext_1.resolveBackendBrainContext;
    const resolveAction = dependencies.resolveBrainAction ?? actionRouter_1.resolveBrainAction;
    const commercialShadowHook = dependencies.commercialShadow?.commercialShadowHook ?? runCommercialShadowEvaluation_1.runCommercialShadowEvaluation;
    const commercialShadowFlags = buildCommercialShadowFlags(dependencies.commercialShadow);
    const commercialOperationalLoopHook = dependencies.commercialOperationalLoop?.commercialOperationalLoopHook ?? operational_loop_1.runCommercialOperationalLoop;
    const commercialOperationalLoopFlags = buildCommercialOperationalLoopFlags(dependencies.commercialOperationalLoop);
    if (request.options.executeActions) {
        return buildFailClosedResponse(request, request, startedAt, [
            {
                code: "ACTION_BLOCKED",
                message: "executeActions=true is not allowed in processInbound.",
                retryable: false
            }
        ], "executeActions=true is not allowed in processInbound.", ["execute_actions_disabled"]);
    }
    const contextRequest = (0, legacyAdapters_1.adaptBrainInboundToContextRequest)(request);
    const contextResponse = await resolveContext(contextRequest, startedAt);
    const requestId = (0, instructions_1.makeBrainRequestId)(request);
    const contextSummary = buildContextSummary(requestId, request, contextResponse);
    let commercialShadowResult = null;
    let commercialOperationalLoopResult = null;
    if (commercialShadowFlags.commercialShadowEnabled) {
        try {
            ...
            commercialShadowResult = (0, createCommercialShadowFailedSafe_1.createCommercialShadowFailedSafe)({
    ...
    let outboxPlanResult = null;
    let outboxPlanWarnings = [];
    ...
    if (shouldRunKnowledgeAgent(request, actionResponse, contextResponse)) {
        try {
            ...
            const agentResponse = await (0, runAgent_1.runAgent)(agentRequest, startedAt);
    ...
    const outboxPlanAttempt = await maybePersistInboundOutboxPlan(request, actionResponse, agentDraft, contextResponse, persistOutboxPlanRequested);
    ...
    if (commercialOperationalLoopFlags.commercialOperationalLoopEnabled) {
        const commercialOperationalLoopInput = buildCommercialOperationalLoopInput(request, contextResponse, commercialShadowResult, startedAt, requestId, commercialOperationalLoopFlags);
    ...
    return buildValidResponse(request, startedAt, contextResponse, actionResponse, agentDraft, executionPlan, outboxPlanResult, outboxPlanWarnings, commercialShadowResult, commercialOperationalLoopResult);
}
