...
function pickRiskLevel(issues) {
    return issues.some((issue) => issue.level === "fatal") ? "blocked" : "high";
}
function pickApprovalRequirement(issues) {
    return issues.some((issue) => issue.level === "fatal") ? "blocked" : "operator_review";
}
function pickErrorCode(issues) {
    const codes = new Set(issues.map((issue) => issue.code));
    if (codes.has("run_id_mismatch"))
        return "run_id_mismatch";
    if (codes.has("unsupported_contract_version"))
        return "unsupported_contract_version";
    if (codes.has("forbidden_key"))
        return "invalid_output";
    if (codes.has("non_serializable_value"))
        return "invalid_output";
    if (codes.has("hard_blocked_action"))
        return "blocked_by_policy";
    return "invalid_output";
}
function createFailedSafeResult(context, input = {}) {
