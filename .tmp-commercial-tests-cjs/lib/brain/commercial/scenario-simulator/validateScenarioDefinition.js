...
exports.validateScenarioDefinition = validateScenarioDefinition;
...
function pushError(errors, code, messageSafe, path) {
    errors.push({ code, messageSafe, path });
}
function scanForUnsafeStrings(value, path, errors) {
    if (typeof value === "string") {
        if ((0, constants_1.containsForbiddenScenarioText)(value)) {
...
function validateStepInput(step, index, errors, tenantId) {
    ...
    if (!isIsoTimestamp(input.now))
        pushError(errors, "invalid_timestamp", "Step timestamp must be valid ISO.", `steps[${index}].input.now`);
    if (!asText(input.correlationId))
        pushError(errors, "missing_correlation_id", "Correlation id is required.", `steps[${index}].input.correlationId`);
    if (!asText(input.tenantId))
        pushError(errors, "missing_tenant_id", "Tenant id is required.", `steps[${index}].input.tenantId`);
    if (!isIsoTimestamp(input.inbound?.receivedAt))
        pushError(errors, "invalid_timestamp", "Inbound timestamp must be valid ISO.", `steps[${index}].input.inbound.receivedAt`);
    ...
    else if (!(0, constants_1.isScenarioRecipientAllowed)(waId)) {
    ...
    for (const [whitelistIndex, recipient] of whitelistedWaIds.entries()) {
        if (!(0, constants_1.isScenarioRecipientAllowed)(recipient)) {
...
function validateScenarioDefinition(value) {
    const errors = [];
    const warnings = [];
    if (!isRecord(value)) {
    ...
    if (!scenarioId)
        pushError(errors, "missing_scenario_id", "Scenario id is required.", "scenarioId");
    if (!name)
        pushError(errors, "missing_name", "Scenario name is required.", "name");
    if (!description)
        pushError(errors, "missing_description", "Scenario description is required.", "description");
    if (!category || !constants_1.SCENARIO_ALLOWED_CATEGORIES.includes(category)) {
    ...
    if (!isRecord(value.metadata)) {
        ...
        if (value.metadata.deterministic !== true)
            pushError(errors, "invalid_metadata", "Scenario metadata must declare deterministic=true.", "metadata.deterministic");
    ...
    for (const [index, step] of steps.entries()) {
        ...
        if (!title)
            pushError(errors, "missing_step_title", "Step title is required.", `steps[${index}].title`);
        if (!isIsoTimestamp(step.now))
            pushError(errors, "invalid_timestamp", "Step now must be valid ISO.", `steps[${index}].now`);
        ...
        tenantId = validateStepInput(step, index, errors, tenantId);
    }
    ...
    for (const [index, expectation] of expectations.entries()) {
        ...
        if (!asText(expectation.expectationId))
            pushError(errors, "missing_expectation_id", "Expectation id is required.", `expectations[${index}].expectationId`);
        ...
        if (!asText(expectation.type))
            pushError(errors, "invalid_expectation_type", "Expectation type is required.", `expectations[${index}].type`);
        const path = asText(expectation.path);
        if (!path || !constants_1.SCENARIO_ALLOWED_PATHS.includes(path)) {
