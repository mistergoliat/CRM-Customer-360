...
exports.validateCommercialTransition = validateCommercialTransition;
function deriveExpectedStage(status) {
...
function allowedTransitions(previous) {
    switch (previous) {
        case null:
        case "new":
...
function blockedReasonList(input, reasons, approvalRequirement) {
    const output = [...reasons];
    if (input.identityResolution.isAmbiguous)
        output.push("identity_conflict");
    if (input.identityResolution.isTerminal && input.previousState && input.previousState.status !== input.resultingState.status)
        output.push("terminal_state");
    if (input.resultingState.aiBlocked)
        output.push("ai_blocked");
    if (input.resultingState.humanOwnerActive)
        output.push("human_owner_active");
    if (input.commercialPolicyResult?.status === "blocked")
        output.push("policy_blocked");
    if (approvalRequirement === "blocked")
        output.push("approval_blocked");
    return [...new Set(output)];
}
function validateCommercialTransition(input) {
    ...
    const requiresHumanReview = approvalRequirement !== "none" ||
    ...
    const evidenceRequired = input.nextAction.type === "prepare_quote" ||
