...
exports.selectNextCommercialAction = selectNextCommercialAction;
function confidenceOrLow(value) {
    return value ?? "low";
}
function approvalOrBlocked(value) {
    if (value === "review" || value === "handoff")
        return "operator_review";
    return value ?? "blocked";
}
function riskOrBlocked(value) {
    return value ?? "blocked";
}
function buildAction(type, reason, input, extras = {}) {
...
function getQuestions(result) {
    ...
    return Array.isArray(questions) ? questions.filter((item) => typeof item === "string" && item.trim().length > 0) : [];
}
...
function selectNextCommercialAction(input) {
    ...
    if (input.salesAgentResult?.outcome === "tool_required" || input.salesAgentResult?.decision.type === "request_tool") {
        return buildAction(missingInformation.length > 0 || questions.length > 0 ? "ask_clarifying_question" : "qualify", missingInformation.length > 0 || questions.length > 0
            ? "Missing information should be collected before advancing."
            : "The opportunity needs qualification before any further step.", input, {
            draftMessage: draftText,
            requiredInformation: missingInformation.length > 0 ? missingInformation : questions
        });
    }
