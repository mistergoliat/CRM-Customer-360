...
exports.runCommercialOperationalLoop = runCommercialOperationalLoop;
const node_crypto_1 = require("node:crypto");
const evaluateCommercialShadowResult_1 = require("../evaluation/evaluateCommercialShadowResult");
const policyConstants_1 = require("../policy/policyConstants");
const loadCommercialState_1 = require("./loadCommercialState");
const persistCommercialState_1 = require("./persistCommercialState");
const reduceCommercialState_1 = require("./reduceCommercialState");
const resolveOpportunityIdentity_1 = require("./resolveOpportunityIdentity");
const selectNextCommercialAction_1 = require("./selectNextCommercialAction");
const validateCommercialTransition_1 = require("./validateCommercialTransition");
...
function createClock(currentTime, provided) {
    if (provided)
        return provided;
...
function createStage(stage, status, startedAt, completedAt, warnings = [], errorCode, counts) {
    return {
        ...
        version: constants_1.COMMERCIAL_OPERATIONAL_LOOP_VERSION,
        counts
    };
}
...
function stableDecisionId(input, resultingStateKey, nextActionType, version) {
...
function mapComparisonStatus(value) {
    if (value === "aligned" || value === "partially_aligned" || value === "divergent" || value === "not_comparable")
        return value;
    return null;
}
...
function buildEvaluationSummary(result) {
    if (!result)
        return null;
    const readinessDecision = result.classification.usefulness === "useful"
...
function buildSafeError(stage, error) {
...
function buildSkippedResult(input, options) {
    ...
    const warnings = uniqueStrings([...(options.warnings ?? []), options.skipReason ?? undefined]);
    return {
        status: options.status,
        mode: input.mode ?? constants_1.COMMERCIAL_OPERATIONAL_LOOP_DEFAULT_MODE,
...
function buildFailedSafeResult(input, args) {
    const clock = createClock(input.currentTime, input.clock);
    const startedAt = clock.toISOString(clock.now());
    const completedAt = clock.toISOString(clock.now());
    const warnings = uniqueStrings([...(args.warnings ?? []), "commercial_state_failed_safe"]);
    const stages = args.stages ??
        [
            createStage(args.failureStage, args.status === "blocked"
                ? "blocked"
                : args.status === "persistence_failed"
                    ? "persistence_failed"
                    : "failed_safe", startedAt, completedAt, warnings, args.error?.code ?? "commercial_state_failed_safe")
        ];
    return {
        status: args.status,
        mode: input.mode ?? constants_1.COMMERCIAL_OPERATIONAL_LOOP_DEFAULT_MODE,
...
function buildDecisionRecord(input, resultingState, nextAction, transitionValidation, identityResolution, stateDiff, warnings) {
    ...
    return {
        ...
        detectedSignals: [...new Set(resultingState.signals)],
...
function buildDefaultInputContext(input, shadowResult) {
    ...
    const commercialEvaluationResult = input.commercialEvaluationResult ??
        (shadowResult
            ? (0, evaluateCommercialShadowResult_1.evaluateCommercialShadowResult)({
...
async function runCommercialOperationalLoop(input) {
    const clock = createClock(input.currentTime, input.clock);
    const startedAtMs = clock.now();
    const startedAt = clock.toISOString(startedAtMs);
    const featureFlags = input.featureFlags;
    const mode = input.mode ?? constants_1.COMMERCIAL_OPERATIONAL_LOOP_DEFAULT_MODE;
    ...
    const loadResult = await (input.storage?.loadCommercialState ?? loadCommercialState_1.loadCommercialState)({
    ...
    const identityResolution = (0, resolveOpportunityIdentity_1.resolveOpportunityIdentity)({
        inboundMessage: input.inboundMessage,
        brainContext: input.brainContext,
        commercialContext: context.commercialContext,
        loadResult,
    ...
    if (identityResolution.status === "no_commercial_signal") {
        return buildSkippedResult(input, {
            ...
            warnings: uniqueStrings([...identityResolution.warnings, "commercial_loop_skipped"]),
            eligible: false,
    ...
    const reductionResult = (0, reduceCommercialState_1.reduceCommercialState)({
    ...
    const nextAction = (0, selectNextCommercialAction_1.selectNextCommercialAction)({
        previousState: identityResolution.selectedState ?? loadResult.activeState ?? null,
    ...
    const transitionValidation = (0, validateCommercialTransition_1.validateCommercialTransition)({
        previousState: identityResolution.selectedState ?? loadResult.activeState ?? null,
        resultingState: reductionResult.resultingState,
        nextAction,
        identityResolution,
    ...
    if (identityResolution.status === "blocked" || transitionValidation.status === "blocked") {
        return buildFailedSafeResult(input, {
            ...
            stages: [
                ...
                createStage("transition_validation", "blocked", transitionValidationStartedAt, transitionValidationCompletedAt, transitionValidation.blockedReasons, "transition_blocked")
            ],
    ...
    const decisionRecord = buildDecisionRecord(input, reductionResult.resultingState, nextAction, transitionValidation, identityResolution, reductionResult.stateDiff, stageWarnings);
    ...
    if (input.featureFlags.commercialStatePersistenceEnabled) {
        ...
        persistenceResult = await (input.storage?.persistCommercialState ?? persistCommercialState_1.persistCommercialState)({
            currentTime: input.currentTime,
            previousState: identityResolution.selectedState ?? loadResult.activeState ?? null,
            resultingState: reductionResult.resultingState,
            identityResolution,
        ...
        if (persistenceResult.status === "conflict" || persistenceResult.status === "failed_safe") {
            return buildFailedSafeResult(input, {
                ...
                warnings: uniqueStrings([...stageWarnings, ...(persistenceResult.warnings ?? [])]),
                eligible: true,
                previousState: identityResolution.selectedState ?? loadResult.activeState ?? null,
                resultingState: reductionResult.resultingState,
                identityResolution,
    ...
    const stages = [
        ...
        createStage("persistence", input.featureFlags.commercialStatePersistenceEnabled ? (persistenceResult.status === "persisted" || persistenceResult.status === "duplicate" ? "completed" : "skipped") : "skipped", persistenceStartedAt, persistenceCompletedAt, persistenceResult.warnings, persistenceResult.status),
