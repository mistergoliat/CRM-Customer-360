...
exports.planCommercialFollowUp = planCommercialFollowUp;
exports.toProposedActionPreview = toProposedActionPreview;
const node_crypto_1 = require("node:crypto");
const adapters_1 = require("../context/adapters");
const constants_1 = require("./constants");
const validateFollowUpPlan_1 = require("./validateFollowUpPlan");
...
function collectStrings(value, output, seen, depth = 0) {
    if (value === null || value === undefined || output.length >= 200 || depth > 4)
        return;
    if (typeof value === "string") {
        const trimmed = value.trim();
        if (trimmed)
...
function buildSearchCorpus(...values) {
    const collected = [];
    const seen = new WeakSet();
    for (const value of values) {
        collectStrings(value, collected, seen);
    }
    const sanitized = (0, adapters_1.sanitizeCommercialObject)(collected.join(" | "));
    return lowerText(sanitized.value?.valueOf() ?? collected.join(" | "));
}
function hasCommercialOpportunityNeed(input) {
    const opportunity = input.opportunity;
    if (!opportunity)
        return false;
    if (constants_1.COMMERCIAL_FOLLOW_UP_ACTIVE_OPPORTUNITY_STATUSES.includes(String(opportunity.status ?? ""))) {
        return true;
    }
    if (asText(opportunity.primaryIntent) && lowerText(opportunity.primaryIntent) !== "unknown")
        return true;
    if (asText(opportunity.currentSummary))
        return true;
    if (asText(opportunity.nextActionType))
        return true;
    if (asText(opportunity.lastActivityAt) || asText(opportunity.lastCustomerMessageId) || asText(opportunity.lastAgentDecisionId))
        return true;
    if (Array.isArray(opportunity.missingRequirements) && opportunity.missingRequirements.length > 0)
        return true;
    if (Array.isArray(opportunity.productInterests) && opportunity.productInterests.length > 0)
        return true;
    if (Array.isArray(opportunity.objections) && opportunity.objections.length > 0)
        return true;
    if (Array.isArray(opportunity.signals) && opportunity.signals.length > 0)
        return true;
    return false;
}
function hasMissingInformationSignal(input) {
    const opportunity = input.opportunity;
    const corpus = buildSearchCorpus(opportunity?.missingRequirements, opportunity?.nextActionType, opportunity?.currentSummary, opportunity?.signals, input.caseContext?.department, input.conversation.lastInboundText, input.lastDecision?.nextActionJson);
...
function isCaseClosed(caseContext) {
    if (!caseContext)
        return false;
    if (asIso(caseContext.closedAt))
        return true;
...
function isOpportunityClosed(opportunity) {
    if (!opportunity)
        return false;
    if (asIso(opportunity.closedAt))
        return true;
    const status = lowerText(opportunity.status);
    return ["won", "lost", "cancelled", "canceled", "archived"].includes(status);
}
function isCustomerReplyAfterAgent(conversation) {
...
function isExpiredSignal(lastDecision) {
    if (!lastDecision)
        return false;
    if (lowerText(lastDecision.decisionStatus) === "expired")
        return true;
    if (isRecord(lastDecision.nextActionJson)) {
        ...
        if (status === "expired")
            return true;
        if (nextAction.expired === true)
            return true;
    }
    return false;
}
function isHighRiskIntent(input) {
    const corpus = buildSearchCorpus(input.opportunity?.primaryIntent, input.opportunity?.currentSummary, input.opportunity?.missingRequirements, input.opportunity?.productInterests, input.opportunity?.objections, input.opportunity?.signals, input.opportunity?.nextActionType, input.caseContext?.department, input.conversation.lastInboundText, input.conversation.lastOutboundText, input.lastDecision?.nextActionJson, input.lastDecision?.decisionStatus);
...
function inferFollowUpIntent(input) {
    const opportunity = input.opportunity;
    const corpus = buildSearchCorpus(opportunity?.primaryIntent, opportunity?.currentSummary, opportunity?.missingRequirements, opportunity?.productInterests, opportunity?.objections, opportunity?.signals, opportunity?.nextActionType, input.caseContext?.department, input.conversation.lastInboundText, input.conversation.lastOutboundText, input.lastDecision?.nextActionJson);
    if (hasMissingInformationSignal(input))
        return "missing_information_followup";
    if (corpus.includes("quote") || corpus.includes("cotiz") || corpus.includes("quotation"))
        return "quote_followup";
    if (corpus.includes("payment") || corpus.includes("checkout") || corpus.includes("pago") || corpus.includes("cobro"))
        return "payment_or_checkout_followup";
    ...
    if (corpus.includes("reactiv") || corpus.includes("retomar") || corpus.includes("volver a contactar"))
        return "reactivation_followup";
...
function extractPriorAttemptCount(lastDecision) {
    if (!lastDecision || !lastDecision.nextActionJson || !isRecord(lastDecision.nextActionJson))
        return 0;
...
function deriveRiskLevel(intent, corpus) {
    if (corpus.includes("legal"))
        return "critical";
    if (intent === "payment_or_checkout_followup")
        return "high";
    if (intent === "quote_followup" || intent === "post_handoff_followup" || intent === "missing_information_followup")
        return "medium";
    if (intent === "no_followup")
        return "unknown";
    return "low";
}
function deriveApprovalRequirement(intent, riskLevel, policy) {
    if (riskLevel === "critical")
        return "blocked";
    if (intent === "payment_or_checkout_followup")
        return "manager_review";
    if (intent === "quote_followup" || intent === "post_handoff_followup")
        return policy.requireOperatorReview ? "operator_review" : "operator_review";
    if (policy.requireOperatorReview)
        return "operator_review";
    if (riskLevel === "high")
        return "manager_review";
    return "none";
}
function buildDraftMessage(intent) {
    if (intent === "no_followup")
        return null;
    return constants_1.COMMERCIAL_FOLLOW_UP_DEFAULT_DRAFTS[intent];
}
...
function finalizePlan(plan, policy) {
    ...
    const digest = (0, node_crypto_1.createHash)("sha256").update(signature).digest("hex");
    const validated = (0, validateFollowUpPlan_1.validateFollowUpPlan)({
        ...plan,
...
function buildInvalidPlan(input, reason) {
    if ("now" in input) {
        return {
            planId: `followup-plan-invalid-${(0, node_crypto_1.createHash)("sha256").update(reason).digest("hex").slice(0, 16)}`,
            ...
            idempotencyKey: `commercial-followup-invalid-${(0, node_crypto_1.createHash)("sha256").update(reason).digest("hex").slice(0, 16)}`,
            executable: false,
            persisted: false,
...
function planCommercialFollowUp(input) {
    ...
    if (opportunity.aiBlocked || opportunity.humanOwnerActive || caseContext?.requiresHuman === true) {
        ...
        const approvalRequirement = normalizedPolicy.requireOperatorReview ? "operator_review" : "blocked";
        const scheduledFor = status === "requires_operator_review" ? plusHours(createdAt, normalizedPolicy.defaultDelayHours) : null;
        const blockReasons = opportunity.aiBlocked ? ["ai_blocked"] : ["human_owner_active"];
        const plan = {
            planId: "",
            opportunityId,
            decisionId,
    ...
    if (highRiskReason) {
        const riskLevel = highRiskReason === "complaint_or_warranty" ? "critical" : "high";
        return buildBlockedPlan({
            planInput: { ...input, policy: normalizedPolicy },
            status: "blocked",
            intent: "no_followup",
    ...
    const cooldownActive = Boolean(lastAgentAt &&
    ...
    const attemptNumber = Math.min(constants_1.COMMERCIAL_FOLLOW_UP_MAX_ATTEMPT_NUMBER, priorAttempts + 1);
    ...
    const status = approvalRequirement === "blocked"
        ? "blocked"
    ...
    const rationale = status === "recommended"
...
function toProposedActionPreview(plan) {
    const statusMap = {
        not_needed: "blocked",
