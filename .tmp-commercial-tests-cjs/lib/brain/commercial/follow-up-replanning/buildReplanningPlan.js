...
exports.buildReplanningPlan = buildReplanningPlan;
...
function canUpdateInPlace(input) {
    if (!input.policy.allowInPlaceScheduleUpdate)
        return false;
    if (input.policy.preserveOriginalAction)
        return false;
    if (!["planned", "scheduled"].includes(input.originalAction.status))
        return false;
    if (["cancelled", "expired", "executed", "rejected", "blocked", "failed", "draft", "requires_review"].includes(input.originalAction.status))
        return false;
    return Boolean(input.schedulingResult.nextScheduledFor);
}
function buildStageReason(input) {
...
function buildReplanAuditEvents(input, reason, replacementActionId) {
...
function buildReplacementDraft(input, reason) {
    ...
    const replacementActionId = (0, constants_1.buildReplacementActionId)({
    ...
    return {
        ...
        idempotencyKey: (0, constants_1.buildReplacementIdempotencyKey)({
            originalActionId: input.originalAction.actionId,
            generation,
            nextScheduledFor,
            reason
...
function buildInPlacePlan(input, reason) {
    const operations = [
        {
...
function buildReplacementPlan(input, reason, planType) {
    if (!input.policy.allowReplacementOnReplan) {
        const fallbackReasons = (0, constants_1.normalizePlanReasons)([reason, "replacement_required"]);
        return {
            planId: (0, constants_1.buildFollowUpMutationPlanId)({
                actionId: input.originalAction.actionId,
                planType: "no_change",
    ...
    if (input.policy.requireAuditEvent) {
        operations.push(...buildReplanAuditEvents(input, reason, replacementActionId).map((event) => ({ type: "append_audit_event", event })));
    }
    const reasons = (0, constants_1.normalizePlanReasons)([reason, planType === "supersede_action" ? "stale_action_context" : "schedule_changed"]);
    return {
        planId: (0, constants_1.buildFollowUpMutationPlanId)({
            actionId: input.originalAction.actionId,
            planType,
...
function buildReplanningPlan(input) {
...
function buildExpirationFallback(input) {
    const reason = "replacement_would_exceed_expiry";
    const operations = [
        {
            type: "update_existing_action",
            patch: {
