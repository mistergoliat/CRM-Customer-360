...
exports.buildAgentActionFromFollowUpPlan = buildAgentActionFromFollowUpPlan;
exports.buildAgentActionFromNextAction = buildAgentActionFromNextAction;
...
function buildCreatedAt(context) {
...
function buildBaseAction(input) {
    ...
    const digest = (0, node_crypto_1.createHash)("sha256").update(JSON.stringify((0, serializeAgentAction_1.sanitizeAgentActionJsonValue)(seed).value ?? null)).digest("hex");
...
function normalizeContext(context) {
    return {
        ...
        lifecycleVersion: asText(context.lifecycleVersion) ?? constants_1.COMMERCIAL_AGENT_ACTION_QUEUE_VERSION,
...
function mapFollowUpStatus(planStatus) {
    switch (planStatus) {
        case "recommended":
            return "proposed";
...
function mapNextActionType(nextActionType) {
...
function deriveActionStatusFromGovernance(input) {
    if (input.actionType === "no_action")
        return "blocked";
    if (input.blockReasons.length > 0 || input.approvalRequirement === "blocked")
        return "blocked";
    if (input.approvalRequirement === "manager_review" || input.approvalRequirement === "operator_review" || input.approvalRequirement === "explicit_operator_approval")
        return "requires_review";
    return input.baseStatus;
}
function buildAgentActionFromFollowUpPlan(input) {
...
function buildAgentActionFromNextAction(input) {
    const normalizedContext = normalizeContext(input.context);
    ...
    const baseStatus = nextAction.approvalRequirement === "blocked" || blockReasons.length > 0
        ? "blocked"
        ...
        : status === "requires_review"
            ...
            : status === "cancelled"
                ...
                : status === "expired"
                    ? "expired"
                    : "allowed";
    const draftPayload = (0, serializeAgentAction_1.sanitizeAgentActionJsonValue)({
        nextAction,
