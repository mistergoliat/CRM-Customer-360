...
exports.decideCommercialReadiness = decideCommercialReadiness;
...
function hasCriticalSafetyIssue(aggregate) {
    return (aggregate.issueCounts["EVAL-SAFETY-SIDE-EFFECT"] ?? 0) > 0 || (aggregate.issueCounts["EVAL-SAFETY-SECRET-EXPOSURE"] ?? 0) > 0;
}
function hasPolicyMissing(aggregate) {
    return (aggregate.issueCounts["EVAL-POLICY-MISSING"] ?? 0) > 0;
}
function hasRuntimeInstability(aggregate, thresholds) {
    return (aggregate.timeoutRate > thresholds.maxTimeoutRate ||
...
function hasPromptProblems(aggregate) {
    return ((aggregate.issueCounts["EVAL-MODEL-GENERIC-OUTPUT"] ?? 0) > 0 ||
        (aggregate.issueCounts["EVAL-MODEL-HARD-BLOCK-PROPOSAL"] ?? 0) > 0 ||
        (aggregate.issueCounts["EVAL-MODEL-LOW-CONFIDENCE"] ?? 0) > 0);
}
function hasPolicyProblems(aggregate) {
    return (aggregate.blockedRate > aggregate.thresholds.maxBlockedRate ||
...
function buildEvidence(aggregate, thresholds) {
    return [
        ...
        .concat(aggregate.datasetMetadata?.synthetic ? ["dataset_synthetic=true"] : ["dataset_synthetic=false"], thresholds.maximumAverageCost >= 0 ? [`cost_budget=${thresholds.maximumAverageCost}`] : [])
        .filter((value) => value.length > 0);
}
function decideCommercialReadiness(aggregate, thresholdOverrides) {
    ...
    if (structuralValidRate >= thresholds.minimumStructurallyValidRate &&
    ...
    const fallbackScore = Math.max(0, Math.min(100, Math.round(structuralValidRate * 25 +
