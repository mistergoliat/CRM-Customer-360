...
exports.buildCommercialEvaluationReport = buildCommercialEvaluationReport;
const evaluationConstants_1 = require("./evaluationConstants");
const decideCommercialReadiness_1 = require("./decideCommercialReadiness");
...
function buildMarkdown(report) {
    ...
    return [
        evaluationConstants_1.COMMERCIAL_EVALUATION_DEFAULT_MARKDOWN_TITLE,
        "",
...
function buildCoverageNotes(aggregate) {
    const notes = [
    ...
    return (0, evaluationUtils_1.uniqueStrings)(notes);
}
function buildTopIssues(aggregate) {
    return aggregate.topIssues.slice(0, 5);
}
function buildTopRules(aggregate) {
    return aggregate.topRules.slice(0, 10);
}
function buildRecommendations(aggregate, readinessDecision) {
...
function buildCommercialEvaluationReport(aggregate) {
    const readiness = (0, decideCommercialReadiness_1.decideCommercialReadiness)(aggregate);
    ...
    const structuredReport = {
        ...
        executiveSummary: readiness.decision === "READY_FOR_CONTROLLED_PILOT"
