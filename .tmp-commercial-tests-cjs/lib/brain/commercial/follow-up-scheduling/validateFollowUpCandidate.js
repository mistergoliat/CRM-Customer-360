...
exports.validateFollowUpCandidate = validateFollowUpCandidate;
...
function validateFollowUpCandidate(input) {
    if (!isRecord(input)) {
    ...
    if (input.activity?.lastInboundAt !== null && input.activity?.lastInboundAt !== undefined && !lastInboundAt)
        return fail("invalid_timestamp");
    ...
    if (input.activity?.lastOutboundAt !== null && input.activity?.lastOutboundAt !== undefined && !lastOutboundAt)
        return fail("invalid_timestamp");
    const lastHumanMessageAt = input.activity?.lastHumanMessageAt === null || input.activity?.lastHumanMessageAt === undefined ? null : parseIso(input.activity.lastHumanMessageAt);
    if (input.activity?.lastHumanMessageAt !== null && input.activity?.lastHumanMessageAt !== undefined && !lastHumanMessageAt)
        return fail("invalid_timestamp");
    ...
    if (input.activity?.lastAiMessageAt !== null && input.activity?.lastAiMessageAt !== undefined && !lastAiMessageAt)
        return fail("invalid_timestamp");
    const opportunityStageChangedAt = input.context?.opportunityStageChangedAt === null || input.context?.opportunityStageChangedAt === undefined
        ? null
    ...
    if (!actionType || !hasAllowedValue(constants_1.FOLLOW_UP_SCHEDULING_SUPPORTED_ACTION_TYPES, actionType)) {
        return fail("unsupported_action_type");
    }
    const status = trimText(action.status);
    if (!status || !hasAllowedValue(constants_1.FOLLOW_UP_SCHEDULING_ALLOWED_STATUSES, status)) {
    ...
    if (!constants_1.FOLLOW_UP_SCHEDULING_RISK_LEVELS.includes(candidate.riskLevel)) {
        candidate.riskLevel = "unknown";
    }
    if (!constants_1.FOLLOW_UP_SCHEDULING_APPROVAL_REQUIREMENTS.includes(candidate.approvalRequirement)) {
