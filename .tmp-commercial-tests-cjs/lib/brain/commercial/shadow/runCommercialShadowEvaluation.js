...
exports.runCommercialShadowEvaluation = runCommercialShadowEvaluation;
const adapters_1 = require("../context/adapters");
const buildCommercialContext_1 = require("../context/buildCommercialContext");
const policy_1 = require("../policy");
const fakeSalesAgentProvider_1 = require("../sales-agent/providers/fakeSalesAgentProvider");
const runSalesAgentDryRun_1 = require("../sales-agent/runSalesAgentDryRun");
const runtimeTypes_1 = require("../sales-agent/runtimeTypes");
const shadowConstants_1 = require("./shadowConstants");
const createCommercialShadowFailedSafe_1 = require("./createCommercialShadowFailedSafe");
...
function buildClock(currentTime, provided) {
    if (provided) {
        return {
            now: provided.now,
...
function stageDuration(startedAt, completedAt) {
    return Math.max(0, new Date(completedAt).getTime() - new Date(startedAt).getTime());
}
function createStage(stage, status, startedAt, completedAt, warnings = [], errorCode, counts) {
    return {
        stage,
        status,
        startedAt,
...
function sanitizeMetadata(metadata) {
    const result = (0, adapters_1.sanitizeCommercialObject)(metadata ?? {});
    return {
        value: result.value ?? {},
...
function detectTechnicalEvent(input) {
...
function buildCommercialInboundMessage(input, currentTime) {
    return {
...
function buildChannelContext(commercialContext, input) {
...
function buildCommercialContextSummary(result) {
    return {
...
function buildRuntimeSummary(runtimeResult) {
    return {
...
function buildGovernedResultSummary(policyResult) {
...
function createSafeError(stage, error, providerName, providerVersion) {
...
function buildContextPayload(input, currentTimeIso) {
...
function buildEarlyResult(input, state, status, skipReason, failureStage, eligible, warnings) {
    const completedAt = state.clock.toISOString(state.clock.now());
    const stages = [
        createStage(failureStage, status === "disabled" || status === "skipped" ? "skipped" : "completed", state.startedAt, completedAt, warnings, status === "disabled" ? "shadow_disabled" : status === "skipped" ? "shadow_skipped" : null)
    ];
    const result = (0, createCommercialShadowFailedSafe_1.createCommercialShadowFailedSafe)({
        input,
        status,
        failureStage,
...
async function runCommercialShadowEvaluation(input) {
    ...
    const overallTimeoutMs = input.options?.timeoutMs ?? shadowConstants_1.COMMERCIAL_SHADOW_DEFAULT_TIMEOUT_MS;
    const contextTimeoutMs = input.options?.contextTimeoutMs ?? shadowConstants_1.COMMERCIAL_SHADOW_CONTEXT_TIMEOUT_MS;
    const runtimeTimeoutMs = input.options?.runtimeTimeoutMs ?? shadowConstants_1.COMMERCIAL_SHADOW_RUNTIME_TIMEOUT_MS;
    const policyTimeoutMs = input.options?.policyTimeoutMs ?? shadowConstants_1.COMMERCIAL_SHADOW_POLICY_TIMEOUT_MS;
    void contextTimeoutMs;
    void runtimeTimeoutMs;
    void policyTimeoutMs;
    if (input.abortSignal?.aborted) {
        return (0, createCommercialShadowFailedSafe_1.createCommercialShadowFailedSafe)({
            input,
            status: "cancelled",
    ...
    state.stages.push(createStage("eligibility", earlyEligibility.eligible ? "completed" : "skipped", state.startedAt, eligibilityCompletedAt, earlyEligibility.warnings, earlyEligibility.eligible ? null : "shadow_skipped"));
    ...
    try {
        ...
        const commercialContext = (0, buildCommercialContext_1.buildCommercialContext)(buildContextPayload(input, startedAt));
        const contextCompletedAt = clock.toISOString(clock.now());
        state.stages.push(createStage("context_builder", commercialContext.status === "success" ? "completed" : commercialContext.status === "insufficient_context" ? "skipped" : "failed_safe", contextStartedAt, contextCompletedAt, commercialContext.warnings, commercialContext.status === "invalid_input" ? "shadow_context_failed" : null, {
        ...
        const runtimeProvider = input.shadowFlags.commercialShadowAllowRealProvider && input.provider
            ? input.provider
            : (0, fakeSalesAgentProvider_1.createFakeSalesAgentProvider)({ behavior: "valid" });
        ...
        const runtimeResult = await (0, runSalesAgentDryRun_1.runSalesAgentDryRun)({
            salesAgentInput: commercialContext.salesAgentInput,
            provider: runtimeProvider,
            options: {
                enabled: true,
                mode: input.runtimeOptions.mode ?? runtimeTypes_1.SALES_AGENT_RUNTIME_DEFAULT_MODE,
                timeoutMs: runtimeTimeoutMs,
                maxInputCharacters: input.runtimeOptions.maxInputCharacters ?? runtimeTypes_1.SALES_AGENT_RUNTIME_MAX_INPUT_CHARACTERS,
                maxOutputCharacters: input.runtimeOptions.maxOutputCharacters ?? runtimeTypes_1.SALES_AGENT_RUNTIME_MAX_OUTPUT_CHARACTERS,
                ...
                dryRun: input.runtimeOptions.dryRun ?? runtimeTypes_1.SALES_AGENT_RUNTIME_DEFAULT_DRY_RUN,
                abortSignal: input.abortSignal ?? null
            },
            expectedRunId: input.correlationId,
            contractVersion: input.contractVersion ?? runtimeTypes_1.SALES_AGENT_CONTRACT_VERSION,
        ...
        state.stages.push(createStage("sales_agent_runtime", runtimeResult.status === "completed_valid" ? "completed" : runtimeResult.status === "timeout" ? "timeout" : runtimeResult.status === "cancelled" ? "cancelled" : "failed_safe", runtimeStartedAt, runtimeCompletedAt, runtimeResult.warnings, runtimeResult.status === "timeout"
            ? "shadow_timeout"
        ...
        if (runtimeResult.status !== "completed_valid" || runtimeResult.validation.status !== "valid") {
            ...
            return (0, createCommercialShadowFailedSafe_1.createCommercialShadowFailedSafe)({
                ...
                error: createSafeError(runtimeResult.validation.status === "failed_safe" ? "output_validation" : "sales_agent_runtime", runtimeResult.error?.message ?? `Sales Agent runtime finished with status ${runtimeResult.status}.`, runtimeResult.provider.name, runtimeResult.provider.version ?? null)
            });
        }
        const runtimeSummary = buildRuntimeSummary(runtimeResult);
        const policyStartedAt = clock.toISOString(clock.now());
        const policyResult = (0, policy_1.evaluateCommercialPolicy)({
            salesAgentResult: runtimeResult.result,
            currentTime: startedAt,
            contractVersion: policy_1.COMMERCIAL_POLICY_CONTRACT_VERSION,
            policyVersion: input.policyVersion ?? policy_1.COMMERCIAL_POLICY_VERSION,
            allowedCapabilities: input.allowedCapabilities,
            featureFlags: input.policyFlags,
            commercialContext: commercialContext.sourceSummary,
        ...
        state.stages.push(createStage("commercial_policy", policyResult.status === "failed_safe" ? "failed_safe" : "completed", policyStartedAt, policyCompletedAt, policyResult.warnings, policyResult.status === "failed_safe" ? "shadow_policy_failed" : null, {
            blockedClaims: policyResult.blockedClaims.length,
            blockedActions: policyResult.blockedActions.length,
            blockedToolRequests: policyResult.blockedToolRequests.length,
            blockedEntityProposals: policyResult.blockedEntityProposals.length
        ...
        const status = policyResult.status === "failed_safe"
            ? "policy_failed"
