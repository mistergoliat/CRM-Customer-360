...
const action_lifecycle_1 = require("../action-lifecycle");
...
function isSafeMessageCandidate(value) {
    return typeof value === "string" && value.trim().length > 0;
}
function mapSandboxBlockReasons(input) {
    const reasons = input.sandboxEvaluation.blockReasons;
    const mapped = [];
    if (reasons.includes("missing_recipient"))
        mapped.push("missing_recipient");
    if (reasons.includes("missing_idempotency_key"))
        mapped.push("missing_idempotency_key");
    if (reasons.includes("action_expired"))
        mapped.push("action_expired");
    if (reasons.includes("unsafe_message") || reasons.includes("unsafe_payload"))
        mapped.push("unsafe_message");
    if (reasons.includes("duplicate_or_conflicting_action"))
        mapped.push("conflicting_action");
    if (reasons.includes("approval_required"))
        mapped.push("approval_not_satisfied");
    if (reasons.includes("risk_too_high"))
        mapped.push("risk_not_allowed");
    if (reasons.includes("unsupported_action_type"))
        mapped.push("unsupported_action_type");
    if (reasons.includes("action_not_ready"))
        mapped.push("action_not_ready");
    if (reasons.includes("policy_blocked"))
        mapped.push("policy_blocked");
    if (reasons.includes("human_owner_active"))
        mapped.push("human_owner_active");
    if (reasons.includes("ai_blocked"))
        mapped.push("ai_blocked");
    if (reasons.includes("case_closed"))
        mapped.push("case_closed");
...
function lifecycleTransitionIsAllowed(input) {
    const status = normalizeStatus(input.action.status);
    if (status === "planned")
        return true;
    if (status !== "proposed" && status !== "approved")
        return false;
    const transition = (0, action_lifecycle_1.validateActionLifecycleTransition)({
        fromStatus: status,
        toStatus: "planned",
...
function evaluateExecutionGate(input) {
    ...
    if (!actionType || !constants_1.EXECUTION_GATE_SUPPORTED_ACTION_TYPES.includes(actionType)) {
        return {
            status: "blocked",
            allowed: false,
    ...
    if (!constants_1.EXECUTION_GATE_ALLOWED_ACTION_STATUSES.includes(status)) {
        return {
            status: "blocked",
            allowed: false,
            blockReasons: ["action_not_ready"],
    ...
    if (riskLevel !== constants_1.EXECUTION_GATE_ALLOWED_RISK_LEVEL || sandboxRisk !== constants_1.EXECUTION_GATE_ALLOWED_RISK_LEVEL) {
        return {
            status: "blocked",
            allowed: false,
    ...
    if (approvalRequirement !== constants_1.EXECUTION_GATE_ALLOWED_APPROVAL_REQUIREMENT || sandboxApproval !== constants_1.EXECUTION_GATE_ALLOWED_APPROVAL_REQUIREMENT) {
        return {
            status: "blocked",
            allowed: false,
            blockReasons: ["approval_not_satisfied"],
