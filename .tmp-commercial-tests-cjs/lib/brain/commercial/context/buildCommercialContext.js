...
exports.buildCommercialContext = buildCommercialContext;
...
function isWarning(value) {
    return COMMERCIAL_WARNING_VALUES.includes(value);
}
...
function validateRequestedMode(value) {
    return value === "minimal" || value === "standard" || value === "recovery";
}
function validateCurrentTime(value) {
    const normalized = (0, adapters_1.normalizeCommercialCurrentTime)(value);
...
function normalizeWarnings(input) {
    ...
    if (!input.hasLatestCustomerMessage)
        warnings.push("missing_latest_customer_message");
    if (!input.hasCustomerReference)
        warnings.push("missing_customer_reference");
    if (!input.hasConversationHistory)
        warnings.push("missing_conversation_history");
    if (!input.channel)
        warnings.push("missing_channel");
    if (input.missingCommercialEntity)
        warnings.push("missing_commercial_entity");
    if (input.staleContext)
        warnings.push("stale_context");
    if (input.identityConflict)
        warnings.push("identity_conflict");
    if (input.aiBlocked)
        warnings.push("ai_blocked");
    if (input.humanOwnershipActive)
        warnings.push("human_owner_active");
    if (!input.supportedContextShape)
        warnings.push("unsupported_context_shape");
    if (input.sanitizationApplied)
        warnings.push("sanitization_applied");
...
function buildIdentityConflict(context, inboundMessage) {
    const groups = [
    ...
    for (const values of groups) {
        const distinct = [...new Set(values.filter((value) => value !== null && value !== undefined).map((value) => String(value)))];
        if (distinct.length > 1)
            return true;
    }
    return false;
}
function mergeMessages(currentInbound, recentMessages, latestOutbound) {
    const combined = [
    ...
    return deduped
        ...
        .slice(0, constants_1.COMMERCIAL_CONTEXT_MAX_RECENT_MESSAGES);
}
function buildInvalidInputResult(message, sourceShape = "unsupported") {
    return {
        status: "invalid_input",
        salesAgentInput: null,
        warnings: ["unsupported_context_shape"],
        sourceSummary: (0, adapters_1.buildCommercialSourceSummary)({
            sourceShape,
        ...
        metadata: (0, adapters_1.buildCommercialMetadata)({
            sourceShape,
...
function buildCommercialContext(input) {
    ...
    if (!currentTime)
        return buildInvalidInputResult("currentTime must be a valid ISO-serializable date.");
    if (!validateRequestedMode(input.requestedMode))
        return buildInvalidInputResult("requestedMode is not supported.");
    if (!input.timezone || !input.timezone.trim())
        return buildInvalidInputResult("timezone is required.");
    const normalizedContextResult = (0, adapters_1.normalizeCommercialBrainContext)(input.brainContext);
    const normalizedInbound = (0, adapters_1.normalizeCommercialInboundMessage)(input.inboundMessage);
    const normalizedContext = normalizedContextResult.context;
    const latestInboundMessage = normalizedInbound.message;
    const inputMetadataResult = (0, adapters_1.sanitizeCommercialObject)(input.metadata ?? null);
    const safeMetadata = {
    ...
    const staleContext = (0, adapters_1.hasStaleCommercialContext)(currentTime, (0, adapters_1.collectCommercialTimestampSignals)({
    ...
    const structuralSignals = (0, adapters_1.buildCommercialStructuralSignals)({
        hasLatestCustomerMessage,
    ...
    const sourceSummary = (0, adapters_1.buildCommercialSourceSummary)({
    ...
    const metadata = (0, adapters_1.buildCommercialMetadata)({
        sourceShape: mergedContext.sourceShape,
    ...
    const salesAgentInput = {
        ...
        platform: normalizedInbound.message?.platform ?? mergedContext.platform,
        department: mergedContext.department,
        identity: (0, adapters_1.buildCommercialIdentityContext)(mergedContext),
        messages: (0, adapters_1.buildCommercialMessageContext)({
        ...
        caseContext: (0, adapters_1.buildCommercialCaseContext)(mergedContext),
        commercial: (0, adapters_1.buildCommercialCommercialContext)(mergedContext),
        structuralSignals,
        availableCapabilities: [...input.availableCapabilities],
        policyContext: (0, adapters_1.normalizeCommercialPolicyContext)(input.policyContext),
        metadata
    };
