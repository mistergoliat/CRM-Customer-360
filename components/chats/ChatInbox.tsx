...
import type { ChatCaseContext, ChatListItem, ChatListResult } from "@/lib/chats";
...
type ChatInboxProps = {
  initialList: ChatListResult;
  initialSelectedCaseId: string | null;
  initialContext: ChatCaseContext | null;
  initialMessages: TimelineEntry[];
...
function bubbleClass(direction: TimelineEntry["direction"]) {
  if (direction === "inbound") return "max-w-[82%] rounded-2xl rounded-bl-sm border border-slate-200 bg-white px-4 py-3";
  if (direction === "system") return "max-w-[82%] rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3";
...
export function ChatInbox({
  ...
  const [list, setList] = useState<ChatListResult>(initialList);
  const [selectedCaseId, setSelectedCaseId] = useState<string | null>(initialSelectedCaseId);
  const [context, setContext] = useState<ChatCaseContext | null>(initialContext);
  const [messages, setMessages] = useState<TimelineEntry[]>(initialMessages);
  ...
  async function fetchList(targetPage = page, targetQuery = query) {
    ...
    const data = (await response.json()) as ChatListResult;
    if (!response.ok) throw new Error(data.error || "No se pudo cargar la lista de chats");
    return data;
  }
  ...
  function pushSelection(nextCaseId: string | null, nextPage = page, nextQuery = query) {
    const params = new URLSearchParams();
  ...
  useEffect(() => {
    const interval = window.setInterval(async () => {
      try {
        ...
        startTransition(() => setList(nextList));
  ...
  async function selectChat(caseId: string) {
  ...
  return (
    <div className="grid min-h-[78vh] gap-4 xl:grid-cols-[360px_minmax(0,1fr)_320px]">
      <section className="hub-card flex min-h-0 flex-col overflow-hidden">
        <div className="border-b border-slate-200 p-4">
          ...
          <form className="flex gap-2" onSubmit={submitSearch}>
            <input
              ...
              placeholder="Buscar cliente, wa_id, orden, factura"
            />
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {list.rows.length === 0 ? (
            <div className="p-4">
              <EmptyState title="Sin chats" description="No hay conversaciones para estos filtros." icon="chat" />
            </div>
          ) : (
            <div className="divide-y divide-slate-100">
              {list.rows.map((chat) => {
                ...
                return (
                  <button
                    key={chat.conversation_case_id}
                    className={`w-full border-l-4 px-4 py-4 text-left transition ${
                      active ? "border-primary-container bg-primary-fixed/40" : "border-transparent hover:bg-slate-50"
                    }`}
                    onClick={() => void selectChat(String(chat.conversation_case_id))}
                  >
                    <div className="mb-2 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-body-md font-semibold text-on-surface">{asText(chat.contact_name, "sin nombre")}</p>
                        <p className="truncate text-label-sm text-slate-500">{asText(chat.wa_id)}</p>
                      </div>
                      <p className="shrink-0 text-label-sm text-slate-500">{formatDateTime(chat.last_message_at || chat.updated_at)}</p>
                    </div>
                    <p className="mb-3 text-body-md text-slate-600">{truncate(chat.last_message, 88)}</p>
                    <div className="flex flex-wrap gap-1">
                      <StatusChip label={asText(chat.status)} />
                      {chat.department ? <StatusChip label={chat.department} tone="gray" /> : null}
                      {chat.service_code ? <StatusChip label={chat.service_code} tone="gray" /> : null}
                      {isTruthy(chat.requires_human) ? <StatusChip label="humano" tone="red" /> : null}
                      {chat.priority ? <StatusChip label={chat.priority} /> : null}
                      <StatusChip label={isTruthy(chat.whatsapp_window_open) ? "24h abierta" : "24h cerrada"} tone={isTruthy(chat.whatsapp_window_open) ? "green" : "amber"} />
                      {chat.message_count !== null ? <StatusChip label={`${chat.message_count} msg`} tone="gray" /> : null}
                    </div>
                  </button>
                );
              })}
      ...
      <section className="hub-card flex min-h-0 flex-col overflow-hidden">
        <div className="border-b border-slate-200 px-5 py-4">
          <div className="flex items-center justify-between gap-3">
            ...
            {loadingChat ? <StatusChip label="actualizando" tone="amber" /> : <StatusChip label="live" tone="green" />}
          </div>
        </div>

        <div ref={viewportRef} className="min-h-0 flex-1 overflow-y-auto bg-slate-50/70 px-5 py-5">
          {messages.length === 0 ? (
            <EmptyState title="Sin mensajes" description="No hay mensajes para esta conversacion." icon="chat_bubble" />
          ) : (
        ...
        <div className="border-t border-slate-200 p-4">
          ...
          <textarea
            className="hub-textarea min-h-24 w-full resize-none bg-slate-50"
            placeholder="Respuesta operacional"
            disabled
            value=""
            readOnly
          />
          ...
          {!writeEnabled ? (
            <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-label-sm text-amber-900">
              {DB_WRITE_DISABLED_MESSAGE}
            </div>
          ) : null}
        </div>
      </section>
      ...
      <aside className="hub-card flex min-h-0 flex-col overflow-hidden">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {!context ? (
            <EmptyState title="Sin contexto" description="Selecciona una conversacion para ver su detalle." icon="info" />
          ) : (
            <div className="grid gap-3">
              {[
                ...
                <div key={String(label)} className="rounded-lg border border-slate-200 bg-white p-3">
                  ...
                  <p className="mt-1 break-words text-body-md font-semibold text-on-surface">{asText(value)}</p>
                </div>
              ))}
            </div>
          )}
