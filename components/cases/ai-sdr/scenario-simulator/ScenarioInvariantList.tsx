...
type ScenarioInvariantListProps = {
  result: ScenarioExecutionResult | null;
};

export function ScenarioInvariantList({ result }: ScenarioInvariantListProps) {
  if (!result) return null;

  const invariants = result.steps.flatMap((step) =>
    step.invariantResults.map((item) => ({
      stepId: step.stepId,
      title: step.title,
      ...item
    }))
  ...
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white/95 p-4 shadow-[0_24px_90px_-45px_rgba(15,23,42,0.35)]">
      ...
      <div className="mt-4 grid gap-3">
        {invariants.map((item) => (
          <div key={`${item.stepId}:${item.invariantId}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <StatusChip label={item.passed ? "passed" : item.severity} tone={item.passed ? "green" : item.severity === "error" ? "red" : "amber"} />
            </div>
