...
export function AiSdrSuggestedReplyCard({ pilot }: { pilot: AiSdrOperatorPilotViewModel }) {
  void React;

  const nextAction = pilot.nextAction;
  if (!nextAction) {
    return <CaseInlineNote tone="info" title="Sin sugerencia disponible" body="No existe una próxima acción lista para mostrar en este caso." />;
  }
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap justify-end gap-2">
          <StatusChip label={nextAction.type} tone={toneForValue(nextAction.type)} />
