  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="CRM"
        title="Oportunidades"
        description="Inbox de pipeline con filtros, tabla densa y panel lateral."
        status="Preview"
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_360px]">
        <SectionCard title="Pipeline" eyebrow="Opportunity inbox" description="Selecciona una fila para abrir el workspace." actions={<StatusChip label="Preview only" tone="amber" />}>
          <div className="mb-4 flex flex-wrap gap-2">
            {["Buscar", "Etapa", "Estado", "Responsable", "Producto", "Prioridad", "Filtros"].map((filter, index) => (
              ...
              <tbody>
                {data.rows.map((row) => (
                  <tr key={row.id} className={row.id === selected?.id ? "bg-primary-fixed/30" : undefined}>
                    ...
                    <td><StatusChip label={row.status} tone={row.status.includes("Revisión") ? "red" : row.status === "Cerrada" ? "green" : "blue"} /></td>
