...
import { makeBrainTraceId, summarizeBrainContext } from "../instructions";
...
import {
  ...
  buildServiceContext,
  normalizeLegacyAgentRunRow,
  normalizeLegacyCaseRow,
  normalizeLegacyMessageRow,
  normalizeLegacyOrderRow,
  normalizeLegacyQueueRow,
  normalizeLegacySuppressionRow
} from "./legacyAdapters";
import type {
  BrainContextResolveRequest,
  BrainContextResolveResponse,
...
type LegacyIdentityGroup = {
  value: string | number | undefined;
  columns: string[];
};

type LegacyFetchResult<T> = {
  rows: T[];
  warnings: string[];
  errors: BrainError[];
};
...
function contextError(message: string, details?: Record<string, unknown>): BrainError {
  return {
...
function pickOrderColumn(columns: string[], candidates: string[]) {
  return candidates.find((candidate) => columns.includes(candidate)) ?? null;
}

function buildIdentityGroups(request: BrainContextResolveRequest, kind: "case" | "message" | "suppression" | "agent_run" | "order" | "queue") {
  const waIdDigits = normalizePhoneLikeId(request.waId);
  const contactId = request.customerRef?.contactId;
  const sharedGroups: LegacyIdentityGroup[] = [
  ...
  if (kind === "agent_run") {
    sharedGroups.push(
      { value: request.messageId, columns: ["conversation_message_id", "message_id"] },
      { value: request.customerRef?.contactId, columns: ["contact_id"] }
    );
  }
  ...
  if (kind === "order") {
    ...
    sharedGroups.push(
      ...
      { value: request.idCustomer, columns: ["id_customer", "customer_id"] },
      { value: request.invoiceNumber, columns: ["invoice_number", "invoice_no"] },
      { value: request.customerRef?.email, columns: ["email"] }
    );
  }
...
function buildWhereClause(columns: string[], groups: LegacyIdentityGroup[]) {
  const clauses: string[] = [];
  const params: unknown[] = [];

  for (const group of groups) {
    if (group.value === undefined || group.value === null || group.value === "") continue;
  ...
  if (clauses.length === 0) return { whereSql: "", params: [] as unknown[] };
  return { whereSql: `WHERE ${clauses.join(" OR ")}`, params };
}

async function fetchLegacyRows<T>(
  tableName: string,
  request: BrainContextResolveRequest,
  kind: "case" | "message" | "suppression" | "agent_run" | "order" | "queue",
  limit: number,
  orderCandidates: string[],
  normalize: (row: Record<string, unknown>, sourceTable: string) => T
): Promise<LegacyFetchResult<T>> {
  ...
  const result = await safeQueryRows<Record<string, unknown>>(sql, [...params, Math.min(Math.max(limit, 1), 50)]);
...
function mergeRowsDescending<T extends { created_at?: string | null; updated_at?: string | null; occurred_at?: string | null }>(rows: T[]) {
...
function buildFallbackResponse(request: BrainContextResolveRequest, startedAt: number, warnings: string[], errors: BrainError[]): BrainContextResolveResponse {
  const emptyCases: BrainLegacyCaseSummary[] = [];
  const emptyMessages: BrainLegacyMessageSummary[] = [];
  const emptyAgentRuns: BrainLegacyAgentRunSummary[] = [];
  const emptyOrders: BrainLegacyOrderSummary[] = [];
  const inputEvent = buildInputEvent(request);
  const customerContext = buildFallbackCustomerContext(request);
  const caseContext = buildCaseContext(emptyCases);
  const businessContext = buildBusinessContext(request, null, null, emptyOrders, request.options.includePostventa, request.options.includeAgentRuns);
  const serviceContext = buildServiceContext(request, caseContext, businessContext);
  const botEligibility = buildBotEligibility({
    messageText: request.messageText,
    customerContext,
    caseContext,
    conversationContext: buildConversationContext(emptyMessages, emptyAgentRuns, {
  ...
  return {
    ...
    resolver_identity: buildResolverIdentity(request, emptyCases, emptyOrders, [null, null]),
    customer_context: customerContext,
    case_context: caseContext,
    conversation_context: buildConversationContext(emptyMessages, emptyAgentRuns, {
      maxMessages: request.options.maxMessages,
      maxAgentRuns: request.options.maxAgentRuns
    }),
...
export function resolveBrainContext(request: BrainNormalizedProcessInboundRequest): BrainResolvedContext {
  const context: BrainResolvedContext = {
    status: "noop",
    source: request.source,
    contextMode: request.contextMode,
    traceId: makeBrainTraceId(request),
    waId: request.waId,
  ...
  return {
    ...context,
    notes: summarizeBrainContext(context).notes
  };
}
...
export async function resolveBackendBrainContext(request: BrainContextResolveRequest, startedAt = Date.now()): Promise<BrainContextResolveResponse> {
  ...
  try {
    const [
      ...
      suppressionResult,
      agentRunsResult,
      postventaQueueResult,
      mantencionesQueueResult,
      ordersResult
    ] = await Promise.all([
      fetchLegacyRows<BrainLegacyCaseSummary>(
        "n8n_conversation_cases",
        request,
        "case",
        request.options.maxCases,
        ["updated_at", "created_at", "closed_at", "last_message_at", "id"],
        normalizeLegacyCaseRow
      ),
      fetchLegacyRows<BrainLegacyMessageSummary>(
        "n8n_wa_inbound_messages",
        request,
        "message",
        request.options.maxMessages,
        ["occurred_at", "created_at", "updated_at", "id"],
        normalizeLegacyMessageRow
      ),
      fetchLegacyRows<BrainLegacyMessageSummary>(
      ...
      fetchLegacyRows<BrainLegacySuppressionSummary>(
      ...
      request.options.includeAgentRuns
        ? fetchLegacyRows<BrainLegacyAgentRunSummary>(
            "n8n_agent_runs",
            request,
            "agent_run",
            request.options.maxAgentRuns,
            ["created_at", "updated_at", "id"],
            normalizeLegacyAgentRunRow
          )
        : Promise.resolve({ rows: [] as BrainLegacyAgentRunSummary[], warnings: [], errors: [] as BrainError[] }),
      request.options.includePostventa
        ? fetchLegacyRows<BrainLegacyQueueSummary>(
            "n8n_postventa_queue",
            request,
            "queue",
            request.options.maxCases,
            ["updated_at", "created_at", "last_inbound_at", "id"],
            (row, sourceTable) => normalizeLegacyQueueRow(row, sourceTable, "postventa_armado")
          )
        : Promise.resolve({ rows: [] as BrainLegacyQueueSummary[], warnings: [], errors: [] as BrainError[] }),
      request.options.includePostventa
        ? fetchLegacyRows<BrainLegacyQueueSummary>(
            ...
            request.options.maxCases,
            ["updated_at", "created_at", "last_inbound_at", "id"],
            (row, sourceTable) => normalizeLegacyQueueRow(row, sourceTable, "postventa_mantencion")
          )
        : Promise.resolve({ rows: [] as BrainLegacyQueueSummary[], warnings: [], errors: [] as BrainError[] }),
      fetchLegacyRows<BrainLegacyOrderSummary>(
        "ps_orders",
        request,
        "order",
        request.options.maxCases,
        ["date_upd", "updated_at", "created_at", "id"],
        normalizeLegacyOrderRow
      )
    ]);
    ...
    const inputEvent = buildInputEvent(request);
    const resolverIdentity = buildResolverIdentity(request, cases, orders, [postventaQueue, mantencionesQueue]);
    ...
    const businessContext = buildBusinessContext(request, postventaQueue, mantencionesQueue, orders, request.options.includePostventa, request.options.includeAgentRuns);
    const serviceContext = buildServiceContext(request, caseContext, businessContext);
    const botEligibility = buildBotEligibility({
      messageText: request.messageText,
      customerContext,
