import { buildFakeWhatsAppProviderMessageId, maskWhatsAppRecipient } from "./constants";
import type {
  WhatsAppHttpClient,
  WhatsAppHttpResponse
} from "./types";
import type { FakeWhatsAppHttpClientScenario } from "./constants";

export type FakeWhatsAppHttpClientCall = {
  requestId: string;
  url: string;
  headers: Record<string, string>;
  body: unknown;
  timeoutMs: number;
  scenario: FakeWhatsAppHttpClientScenario;
};

export type FakeWhatsAppHttpClientSafeLogEntry = {
  requestId: string;
  commandId: string;
...
export type FakeWhatsAppHttpClientConfig = {
  scenarioByRequestId?: Record<string, FakeWhatsAppHttpClientScenario>;
  scenarioByCommandId?: Record<string, FakeWhatsAppHttpClientScenario>;
  scenarioByIdempotencyKey?: Record<string, FakeWhatsAppHttpClientScenario>;
  defaultScenario?: FakeWhatsAppHttpClientScenario;
  explicitRetryAfterSeconds?: number | null;
};
...
function getRecordField(value: Record<string, unknown> | null, key: string): unknown {
  if (!value) return undefined;
  return Object.prototype.hasOwnProperty.call(value, key) ? value[key] : undefined;
}

function scenarioStatusCode(scenario: FakeWhatsAppHttpClientScenario): number {
  switch (scenario) {
    case "accepted":
...
function scenarioBody(input: {
  ...
  const providerMessageId = buildFakeWhatsAppProviderMessageId({
...
function resolveScenario(
  ...
  config: FakeWhatsAppHttpClientConfig
): FakeWhatsAppHttpClientScenario {
  const body = isRecord(input.body) ? input.body : null;
  const metadata = isRecord(getRecordField(body, "metadata")) ? (getRecordField(body, "metadata") as Record<string, unknown>) : null;
  ...
  const idempotencyKey =
    ...
    input.requestId;
  return (
    config.scenarioByRequestId?.[input.requestId] ??
...
export class FakeWhatsAppHttpClient implements WhatsAppHttpClient {
  readonly rawRequestsForTests: Array<{
    url: string;
    headers: Record<string, string>;
    body: unknown;
    timeoutMs: number;
  ...
  readonly safeRequestLog: FakeWhatsAppHttpClientSafeLogEntry[] = [];

  private readonly config: FakeWhatsAppHttpClientConfig;

  constructor(config: FakeWhatsAppHttpClientConfig = {}) {
    this.config = { ...config };
  }
  ...
  async postJson(input: {
    ...
    const body = isRecord(input.body) ? input.body : null;
    const metadata = isRecord(getRecordField(body, "metadata")) ? (getRecordField(body, "metadata") as Record<string, unknown>) : null;
    const text = isRecord(getRecordField(body, "text")) ? (getRecordField(body, "text") as Record<string, unknown>) : null;
    const commandId = asTrimmedString(getRecordField(body, "commandId")) ?? asTrimmedString(getRecordField(metadata, "commandId")) ?? input.requestId;
    ...
    const safeLogEntry: FakeWhatsAppHttpClientSafeLogEntry = {
      requestId: input.requestId,
      commandId,
      idempotencyKey,
