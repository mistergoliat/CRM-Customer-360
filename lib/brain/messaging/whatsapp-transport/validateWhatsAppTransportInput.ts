...
import type { WhatsAppTransportConfig, WhatsAppTransportSendInput, WhatsAppTransportValidationResult } from "./types";
...
function isRawJsonLikeMessage(message: string) {
  const trimmed = message.trim();
  if (!trimmed) return false;
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) return true;
  if (trimmed.includes('"messaging_product"') || trimmed.includes('"recipient_type"') || trimmed.includes('"preview_url"')) return true;
  try {
...
function hasUnresolvedPlaceholder(message: string) {
  return /\{\{[^}]+\}\}|\$\{[^}]+\}|<<[^>]+>>/.test(message);
}

function hasCredentialLikeContent(message: string) {
...
function buildFailureResult(input: {
  ...
  errorCode: WhatsAppTransportValidationResult["errorCode"];
  ...
  return {
    ...
    warnings: [input.errorMessageSafe, ...(input.warnings ?? [])].filter((value): value is string => Boolean(value))
  };
}

function isExactWhitelistMatch(recipient: string, allowedRecipients: string[]) {
  return allowedRecipients.map((value) => normalizeWhatsAppRecipientDigits(value)).filter((value): value is string => Boolean(value)).includes(recipient);
}
...
export function validateWhatsAppTransportInput(
  ...
  const recipientMasked = maskWaId(input.recipient);
  ...
  const requestId = commandId && idempotencyKey ? buildWhatsAppRequestId(commandId, idempotencyKey) : null;
