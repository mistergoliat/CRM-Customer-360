...
function buildAuditEvent(
  input: {
    ...
    eventType: OutboxWorkerAuditEventDraft["eventType"];
    metadata: Record<string, unknown>;
  },
  replacementTransportResult?: MessageTransportResult | null
) {
  return {
    eventId: buildOutboxAuditEventId({
    ...
    createdAt: input.now
  } satisfies OutboxWorkerAuditEventDraft;
}
...
function buildBasePlan(input: {
  ...
  planType: OutboxWorkerPlanType;
  reason: OutboxWorkerPlanReason;
  expectedStatuses: OutboxMessageRecord["status"][];
  nextStatus: OutboxMessageRecord["status"];
  transportResult: MessageTransportResult | null;
  patch: Partial<OutboxWorkerMutationPlan["patch"]>;
  auditEvent: OutboxWorkerAuditEventDraft | null;
}): OutboxWorkerMutationPlan {
  const plan = {
    planId: buildOutboxWorkerPlanId({
      rowId: input.record.rowId,
      commandId: input.record.commandId,
      attemptCount: input.record.attemptCount,
      planType: input.planType,
    ...
    planKey: buildOutboxWorkerPlanKey({
      rowId: input.record.rowId,
      commandId: input.record.commandId,
      attemptCount: input.record.attemptCount,
      planType: input.planType,
    ...
    createdAt: input.now
  } satisfies OutboxWorkerMutationPlan;

  return plan;
}

function buildProcessingPlan(input: OutboxWorkerPlanInput): OutboxWorkerMutationPlan {
...
function buildReleasedClaimPlan(input: OutboxWorkerPlanInput, reason: OutboxWorkerPlanReason): OutboxWorkerMutationPlan {
  const auditEvent = buildAuditEvent({
    now: input.now,
    record: input.record,
...
function buildDeliveredPlan(input: OutboxWorkerPlanInput, transportResult: MessageTransportResult, reason: OutboxWorkerPlanReason): OutboxWorkerMutationPlan {
...
function buildRetryPlan(input: OutboxWorkerPlanInput, transportResult: MessageTransportResult, retryAt: string, reason: OutboxWorkerPlanReason): OutboxWorkerMutationPlan {
  const auditEvent = buildAuditEvent({
    now: input.now,
    record: input.record,
  ...
  return buildBasePlan({
    ...
    patch: {
      ...
      lastErrorMessageSafe: sanitizeOutboxWorkerErrorMessage(transportResult.errorMessageSafe),
      providerMessageId: transportResult.providerMessageId
    },
    auditEvent
  });
...
function buildDeadLetterPlan(input: OutboxWorkerPlanInput, transportResult: MessageTransportResult | null, reason: OutboxWorkerPlanReason): OutboxWorkerMutationPlan {
  const auditEvent = buildAuditEvent({
    now: input.now,
    record: input.record,
  ...
  return buildBasePlan({
    ...
    patch: {
      ...
      lastErrorMessageSafe: sanitizeOutboxWorkerErrorMessage(transportResult?.errorMessageSafe ?? "Moved to dead letter."),
...
function buildExpiredPlan(input: OutboxWorkerPlanInput): OutboxWorkerMutationPlan {
  const auditEvent = buildAuditEvent({
    now: input.now,
    record: input.record,
...
function buildFailedPlan(input: OutboxWorkerPlanInput, reason: OutboxWorkerPlanReason): OutboxWorkerMutationPlan {
  const auditEvent = buildAuditEvent({
    now: input.now,
    record: input.record,
...
function buildNoChangePlan(input: OutboxWorkerPlanInput, reason: OutboxWorkerPlanReason): OutboxWorkerMutationPlan {
  return buildBasePlan({
    now: input.now,
    record: input.record,
    planType: "no_change",
...
export function buildOutboxWorkerPlan(input: OutboxWorkerPlanInput): OutboxWorkerMutationPlan {
  ...
  if (!transportResult) {
    if (input.evaluation.decision === "expire") return buildExpiredPlan(input);
    if (input.evaluation.decision === "dead_letter") return buildDeadLetterPlan(input, null, input.evaluation.reasons[0] ?? "retry_exhausted");
    return buildNoChangePlan(input, input.evaluation.reasons[0] ?? "idempotent_plan_reused");
  }
  ...
  if (transportResult.status === "temporary_failure" || transportResult.status === "rate_limited" || transportResult.status === "timeout") {
    const retryReason =
      transportResult.status === "temporary_failure"
        ? "transport_temporary_failure"
        : transportResult.status === "rate_limited"
          ? "transport_rate_limited"
          : "transport_timeout";
    const retrySchedule = calculateOutboxRetrySchedule({
      now: input.now,
      attemptCount: input.record.attemptCount,
      maxAttempts: input.record.maxAttempts,
...
export function buildFinalOutboxWorkerPlan(
  input: Omit<OutboxWorkerPlanInput, "phase"> & { transportResult: MessageTransportResult | null }
): OutboxWorkerMutationPlan {
  return buildOutboxWorkerPlan({ ...input, phase: "final" });
}

export function buildProcessingOutboxWorkerPlan(input: Omit<OutboxWorkerPlanInput, "phase" | "transportResult">): OutboxWorkerMutationPlan {
  return buildOutboxWorkerPlan({ ...input, transportResult: null, phase: "processing" });
}

export function buildSkippedOutboxWorkerPlan(input: Omit<OutboxWorkerPlanInput, "phase" | "transportResult">): OutboxWorkerMutationPlan {
  return buildOutboxWorkerPlan({ ...input, transportResult: null, phase: "skip" });
}

export function isRetryableTransportResult(result: MessageTransportResult) {
  return result.status === "temporary_failure" || result.status === "rate_limited" || result.status === "timeout" || isRetryableTransportErrorCode(result.errorCode);
}

export function isPermanentTransportResult(result: MessageTransportResult) {
  return result.status === "permanent_failure" || isPermanentTransportErrorCode(result.errorCode);
}
