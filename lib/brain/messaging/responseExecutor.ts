...
import { getMetaSendAdapterStatus } from "./metaSendAdapter";
import { buildOutboxPreview, createOutboxPlannedRecord, type BrainOutboxPersistResult } from "./outbox";
import { buildDedupeKey, checkDuplicateNoop } from "./dedupe";
import type {
  BrainExecutionActionPolicy,
  BrainExecutionBotEligibility,
  BrainExecutionBlockReason,
  BrainExecutionPlan,
  BrainExecutionSource,
  BrainExecuteRequest,
  BrainExecuteResponse
} from "./types";
...
function blockReason(code: string, message: string, retryable = false, details?: Record<string, unknown>): BrainExecutionBlockReason {
  return {
    code,
    message,
    retryable,
...
function normalizeSource(value: unknown): BrainExecutionSource | null {
  return value === "brain" || value === "n8n" || value === "operator" ? value : null;
}

function normalizeActionType(value: unknown): BrainExecuteRequest["action"]["type"] | null {
...
function normalizeActionPolicy(input: unknown): BrainExecutionActionPolicy | undefined {
...
function normalizeBotEligibility(input: unknown): BrainExecutionBotEligibility | undefined {
  if (!isRecord(input)) return undefined;
  return {
...
function makeValidationError(message: string, details?: Record<string, unknown>): BrainError {
  return error(message, details);
}

function validationReasonCode(error: BrainError) {
  const field = typeof error.details?.field === "string" ? error.details.field : null;
  if (field === "dryRun") return "dry_run_required";
  if (field === "executeActions") return "execute_actions_disabled";
  if (field === "source") return "source_required";
  if (field === "action.type") return "action_type_required";
  if (field === "context.waId") return "wa_id_required";
  if (field === "context.phoneNumberId") return "phone_number_id_required";
...
function collectPolicyBlockedReasons(policy?: BrainExecutionActionPolicy) {
  const reasons = new Set<string>();
  const raw = policy?.blockedReasons ?? policy?.blocked_reasons ?? [];
  for (const reason of raw) reasons.add(reason);
  return reasons;
}

function collectEligibilityBlockedReasons(botEligibility?: BrainExecutionBotEligibility) {
  const reasons = new Set<string>();
  const raw = botEligibility?.blockedReasons ?? botEligibility?.blocked_reasons ?? [];
  for (const reason of raw) reasons.add(reason);

  if (botEligibility?.suppressionActive || botEligibility?.suppression_active) reasons.add("suppression_active");
  if (botEligibility?.recentManualReply || botEligibility?.recent_manual_reply) reasons.add("recent_manual_reply");
  ...
  if (botEligibility?.activeHumanCase || botEligibility?.active_human_case) reasons.add("active_human_case");
  if (botEligibility?.openCaseWaitingHuman || botEligibility?.open_case_waiting_human) reasons.add("open_case_waiting_human");
...
function buildInvalidPlan(
  ...
  actionType: BrainExecutionPlan["type"] = request.action.type
): BrainExecutionPlan {
  return {
    type: actionType,
    status,
...
function canAutoReply(policy?: BrainExecutionActionPolicy, botEligibility?: BrainExecutionBotEligibility) {
  return Boolean(
    policy?.allowedToAutoReply ??
...
function canHumanHandoff(policy?: BrainExecutionActionPolicy, botEligibility?: BrainExecutionBotEligibility) {
  return Boolean(policy?.canHumanHandoff ?? botEligibility?.requiresHuman === false);
}

function canCaseMutation(policy?: BrainExecutionActionPolicy) {
  return Boolean(policy?.canCaseMutation);
}

function buildValidationErrors(request: BrainExecuteRequest) {
  ...
  if (request.action.type === "send_whatsapp_message") {
    ...
    if (!context.messageText || !context.messageText.trim()) {
      ...
      errors.push(
        makeValidationError("messageText exceeds the maximum allowed length.", {
...
function buildActionBlockedReasons(request: BrainExecuteRequest) {
  ...
  if (request.action.type === "send_whatsapp_message") {
    if (!canAutoReply(request.actionPolicy, request.botEligibility)) blockedReasons.add("auto_reply_not_allowed");
    ...
    for (const reason of eligibilityReasons) blockedReasons.add(reason);
    for (const reason of policyReasons) blockedReasons.add(reason);
  } else if (request.action.type === "update_case" || request.action.type === "handoff" || request.action.type === "close_case") {
    if (!canCaseMutation(request.actionPolicy)) blockedReasons.add("case_mutation_disabled");
    if (request.action.type === "handoff" && !canHumanHandoff(request.actionPolicy, request.botEligibility)) blockedReasons.add("human_handoff_disabled");
    if (request.action.type === "close_case" && !canCaseMutation(request.actionPolicy)) blockedReasons.add("case_close_disabled");
    for (const reason of eligibilityReasons) blockedReasons.add(reason);
    for (const reason of policyReasons) blockedReasons.add(reason);
...
function buildPlanReason(request: BrainExecuteRequest, blockedReasons: string[], errors: BrainError[]) {
  if (errors.length > 0) return errors[0]?.message ?? "Invalid execution request.";
  if (request.action.type === "no_action") return "no_action";
  if (request.action.type === "send_whatsapp_message" && blockedReasons.length === 0) return "dry_run_only";
  if (blockedReasons.length > 0) return blockedReasons[0] ?? "action_blocked";
  return "dry_run_only";
}

function buildMetaPreview(request: BrainExecuteRequest) {
  const context = request.context ?? {};
  if (request.action.type !== "send_whatsapp_message") return null;
  ...
  if (!waId || !text.trim()) return null;
  return buildMetaWhatsAppTextPayloadPreview({ waId, messageText: text });
}

function buildResponseFromRequest(request: BrainExecuteRequest, startedAt = Date.now()): BrainExecuteResponse {
  ...
  const dedupeCheck = checkDuplicateNoop({
    source: request.source,
  ...
  const outboxPreview = request.action.type === "send_whatsapp_message" && !invalidRequest
    ...
    : null;
  const planStatus: BrainExecutionPlan["status"] = invalidRequest
    ? noAction
      ? "noop"
      : "blocked"
    : noAction
      ...
      : blockedReasons.length > 0
        ...
        : "planned";
  const executionPlan =
    invalidRequest && request.action.type === "send_whatsapp_message"
      ? buildInvalidPlan(
          request,
          buildPlanReason(request, blockedReasons, errors),
          blockedReasons.length > 0 ? blockedReasons : validationBlockedReasons,
          true,
          "noop",
          "no_action"
        )
      : request.action.type === "send_whatsapp_message"
        ...
        : buildInvalidPlan(
            request,
            request.action.type === "no_action" ? "no_action" : blockedReasons[0] ?? "dry_run_only",
            blockedReasons,
            requiresHuman,
            planStatus
          );
  ...
  return {
    ...
    metadata: {
      ...
      send_adapter_status: getMetaSendAdapterStatus()
    }
  };
}

export function normalizeBrainExecuteRequest(input: unknown): BrainValidationResult<BrainExecuteRequest> {
  if (!isRecord(input)) {
    return {
      ok: false,
      value: null,
  ...
  if (!source) errors.push(makeValidationError("source must be one of brain, n8n or operator.", { field: "source" }));
  if (!actionType) errors.push(makeValidationError("action.type is required and must be valid.", { field: "action.type" }));
  if (!options?.dryRun) errors.push(makeValidationError("dryRun=true is required.", { field: "dryRun" }));
  if (options?.executeActions) errors.push(makeValidationError("executeActions=true is not allowed.", { field: "executeActions" }));

  if (actionType === "send_whatsapp_message") {
    if (!context?.waId) errors.push(makeValidationError("context.waId is required.", { field: "context.waId" }));
    if (!context?.phoneNumberId) errors.push(makeValidationError("context.phoneNumberId is required.", { field: "context.phoneNumberId" }));
    if (!context?.messageText || !context.messageText.trim()) {
      errors.push(makeValidationError("context.messageText is required.", { field: "context.messageText" }));
    } else if (context.messageText.trim().length > MAX_MESSAGE_TEXT_CHARS) {
      errors.push(
        makeValidationError("context.messageText exceeds the maximum allowed length.", {
          field: "context.messageText",
          maxMessageTextChars: MAX_MESSAGE_TEXT_CHARS
        })
      );
...
export function evaluateBrainExecution(request: BrainExecuteRequest, startedAt = Date.now()): BrainExecuteResponse {
  return buildResponseFromRequest(request, startedAt);
}

function buildPersistedOutboxPayload(
  request: BrainExecuteRequest,
  ...
  const conversationCaseId =
    request.context?.conversationCaseId ?? asOptionalStringOrNumber(actionPayload?.conversationCaseId) ?? null;
...
async function maybePersistOutboxPlan(request: BrainExecuteRequest, response: BrainExecuteResponse): Promise<BrainOutboxPersistResult | null> {
  if (!request.persistOutboxPlan) return null;
  if (request.action.type !== "send_whatsapp_message") return null;
  if (response.execution_plan.status === "noop") return null;
  ...
  return createOutboxPlannedRecord({
    dedupeKeyInput: {
      source: request.source,
      actionType: request.action.type,
      channel: "whatsapp",
...
export async function resolveBrainExecution(input: unknown, startedAt = Date.now()): Promise<BrainExecuteResponse> {
  ...
  if (!normalizedResult.ok) {
    ...
    const fallbackRequest: BrainExecuteRequest = {
      requestId: undefined,
      source: "brain",
      dryRun: false,
    ...
    return {
      ...response,
      ok: false,
      execution_plan: buildInvalidPlan(
        fallbackRequest,
        normalizedResult.errors[0]?.message ?? "Invalid execution request.",
        blockedReasons.length > 0 ? blockedReasons : ["invalid_input"],
        true,
        "noop"
      ),
      blocked_reasons: blockedReasons.length > 0 ? blockedReasons : ["invalid_input"],
      block_reasons: normalizedResult.errors.map((item) =>
        blockReason(item.code, item.message, item.retryable, item.details)
      ),
