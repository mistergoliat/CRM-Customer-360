...
import type { BrainAgentDecision, BrainAgentOutputEnvelope } from "../agents/types";
import type { BrainModelAdapterRequest, BrainModelAdapterResponse, BrainModelName } from "./types";

function mapKnowledgeDecisionToGenericDecision(decision: string): BrainAgentDecision {
  if (decision === "answer") return "reply";
  if (decision === "abstain") return "no_action";
  if (decision === "handoff_recommended") return "handoff";
  if (decision === "route_to_sales" || decision === "route_to_sac" || decision === "route_to_postventa") return "handoff";
  return "blocked";
}

function buildGenericBlockedOutput(request: BrainModelAdapterRequest, reason: string, safetyFlags: string[]): BrainAgentOutputEnvelope {
  return {
...
function buildGenericOutputFromKnowledge(request: BrainModelAdapterRequest, draft: Awaited<ReturnType<typeof runKnowledgeAgent>>): BrainAgentOutputEnvelope {
  return {
    outputSchema: "brain.agent.output.v1",
    agentName: request.agentName,
    agentVersion: request.agentVersion,
...
export async function runBrainModelAdapter(request: BrainModelAdapterRequest): Promise<BrainModelAdapterResponse> {
  if (request.agentName === "knowledge") {
    const draft = await runKnowledgeAgent(
      {
        requestId: undefined,
        inputEvent: {
    ...
    const modelName: BrainModelName = draft.metadata.modelName === "real" ? "real" : draft.metadata.modelName === "mock" ? "mock" : "disabled";
    return {
      ok: draft.ok,
      modelName,
