...
import type { CommercialPolicyIssue, CommercialPolicyResult } from "../policy";
import type {
  SalesAgentClaim,
...
import type {
  ...
  CommercialShadowReviewEvaluation,
  CommercialShadowReviewIdentifiers,
  CommercialShadowReviewInput,
  CommercialShadowReviewInvariants,
  CommercialShadowReviewObservability,
  CommercialShadowReviewPolicyTrace,
  CommercialShadowReviewSummary,
  CommercialShadowReviewUnavailableInput,
  CommercialShadowReviewViewModel
} from "./types";
...
function buildIdentifiers(input: CommercialShadowReviewInput): CommercialShadowReviewIdentifiers {
  return {
...
function createEmptyInvariants(): CommercialShadowReviewInvariants {
  return {
    shadow: true,
    dryRun: true,
...
function mapClaim(claim: SalesAgentClaim, status: CommercialShadowReviewViewModel["claims"]["detected"][number]["status"], reason: string | null): CommercialShadowReviewViewModel["claims"]["detected"][number] {
  return {
    status,
    type: claim.type,
...
function mapAction(
  ...
  status: CommercialShadowReviewViewModel["actions"]["proposed"][number]["status"],
  reason: string | null
): CommercialShadowReviewViewModel["actions"]["proposed"][number] {
  return {
    status,
    type: action.type,
...
function mapToolRequest(
  request: SalesAgentToolRequest,
  status: CommercialShadowReviewViewModel["toolRequests"]["proposed"][number]["status"],
  available: boolean | null,
  reason: string | null
): CommercialShadowReviewViewModel["toolRequests"]["proposed"][number] {
  return {
    status,
    tool: request.tool,
    ...
    requiredInputs: isRecord(request.requiredInputs) ? sanitizeCommercialObject(request.requiredInputs).value ?? {} : {}
  };
}

function mapEntityProposal(
  proposal: SalesAgentEntityProposal,
  status: CommercialShadowReviewViewModel["entityProposals"]["proposed"][number]["status"],
  reason: string | null
): CommercialShadowReviewViewModel["entityProposals"]["proposed"][number] {
  return {
    status,
...
function mapPolicyIssues(issues: CommercialPolicyIssue[]) {
  return issues.map((issue) => ({
    ...
    details: isRecord(issue.details) ? sanitizeCommercialObject(issue.details).value ?? {} : null
  }));
}

function buildPolicyTrace(policyResult: CommercialPolicyResult | null, evaluationResult: CommercialEvaluationResult | null, shadowResult: CommercialShadowResult): CommercialShadowReviewPolicyTrace {
  ...
  return {
    ...
    versions: {
      ...
      policyVersion: policyResult?.metadata.policyVersion ?? COMMERCIAL_POLICY_VERSION,
...
function buildAvailableWarnings(
...
function buildAvailableInvariants(shadowResult: CommercialShadowResult): CommercialShadowReviewInvariants {
  const violations = [
...
function buildAvailableMetadata(input: CommercialShadowReviewAvailableInput, shadowResult: CommercialShadowResult, policyResult: CommercialPolicyResult | null, evaluationResult: CommercialEvaluationResult | null) {
...
function buildUnavailableMetadata(input: CommercialShadowReviewUnavailableInput | CommercialShadowReviewErrorInput) {
  const sanitized = sanitizeMetadata(input.metadata ?? null);
  return {
    ...sanitized.value,
    review: {
...
function buildUnavailableReview(input: CommercialShadowReviewUnavailableInput): CommercialShadowReviewViewModel {
...
function buildErrorReview(input: CommercialShadowReviewErrorInput): CommercialShadowReviewViewModel {
  const warnings = uniqueStrings([... (input.warnings ?? [])]);
...
function mapClaimArrays(shadowResult: CommercialShadowResult, policyResult: CommercialPolicyResult | null) {
...
function mapActionArrays(shadowResult: CommercialShadowResult, policyResult: CommercialPolicyResult | null) {
...
function mapToolRequestArrays(shadowResult: CommercialShadowResult, policyResult: CommercialPolicyResult | null) {
...
function mapEntityProposalArrays(shadowResult: CommercialShadowResult, policyResult: CommercialPolicyResult | null) {
...
function buildAvailableReview(input: CommercialShadowReviewAvailableInput): CommercialShadowReviewViewModel {
...
export function buildCommercialShadowReview(input: CommercialShadowReviewInput): CommercialShadowReviewViewModel {
