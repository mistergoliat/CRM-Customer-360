import { COMMERCIAL_CONTEXT_MAX_RECENT_MESSAGES, COMMERCIAL_CONTEXT_STALE_THRESHOLD_MS, COMMERCIAL_CONTEXT_VERSION } from "../constants";
import type {
  SalesAgentCaseContext,
  SalesAgentCommercialContext,
  SalesAgentIdentityContext,
  SalesAgentMessageContext,
  SalesAgentMessageSnapshot,
  SalesAgentPolicyContext,
...
export type NormalizedCommercialBrainContext = {
  sourceShape: string;
...
export type NormalizedCommercialInboundMessage = {
  sourceShape: string;
  supportedShape: boolean;
  sanitizationApplied: boolean;
  sanitizedFields: string[];
  message: SalesAgentMessageSnapshot | null;
  metadata: Record<string, unknown>;
};
...
function getValue(value: unknown, path: PathKey[]): unknown {
...
function getRecord(value: unknown, path: PathKey[]): SerializableRecord | null {
...
function toSerializableId(value: unknown): string | number | null {
...
function normalizeDirection(value: unknown): SalesAgentMessageSnapshot["direction"] {
  const text = asString(value)?.toLowerCase() ?? "";
  if (text === "inbound" || text === "outbound" || text === "manual" || text === "system") return text;
  return null;
}
...
function sanitizeValue(
  value: unknown,
  state: { applied: boolean; seen: WeakSet<object>; removedKeys: string[] }
): unknown {
...
export function sanitizeCommercialObject(value: unknown): {
  value: Record<string, unknown> | null;
  applied: boolean;
  sanitizedFields: string[];
} {
  const state = { applied: false, seen: new WeakSet<object>(), removedKeys: [] as string[] };
...
function sanitizePolicyContext(policyContext: unknown): SalesAgentPolicyContext | undefined {
  const sanitized = sanitizeCommercialObject(policyContext);
  return sanitized.value ? (sanitized.value as SalesAgentPolicyContext) : undefined;
}

function normalizeMessage(
  input: unknown,
  fallbackDirection: SalesAgentMessageSnapshot["direction"] = null
): {
  message: SalesAgentMessageSnapshot | null;
  sanitizationApplied: boolean;
} {
...
function normalizeMessageArray(messages: unknown) {
  if (!Array.isArray(messages)) {
    return { messages: [] as SalesAgentMessageSnapshot[], sanitizationApplied: false, sanitizedFields: [] as string[] };
  }

  const output: SalesAgentMessageSnapshot[] = [];
...
function sortMessagesDescending(messages: SalesAgentMessageSnapshot[]) {
...
function hasSupportableContextShape(input: unknown) {
...
function extractCustomerCandidate(context: SerializableRecord | null) {
...
function normalizeCommercialContext(input: unknown): {
  context: NormalizedCommercialBrainContext;
  warnings: string[];
} {
  const warnings: string[] = [];
  const supportedContextShape = hasSupportableContextShape(input);
  if (!supportedContextShape) warnings.push("unsupported_context_shape");
  ...
  const orderContext = sanitizeCommercialObject(
    getValue(root, ["business_context", "ps_orders", 0]) ??
  ...
  const recentMessageSource =
    getValue(conversationContext ?? root, ["recent_messages"]) ??
  ...
  const latestInboundMessage =
    normalizeMessage(getValue(root, ["latestInboundMessage"])).message ??
  ...
  const latestOutboundMessage =
    normalizeMessage(getValue(root, ["latestOutboundMessage"]), "outbound").message ??
  ...
  const waId = pickFirstString(
    getValue(customerContext, ["wa_id"]),
    getValue(customerContext, ["waId"]),
    latestInboundMessage?.waId,
    latestOutboundMessage?.waId
  );
  const phoneNumberId = pickFirstString(
    getValue(customerContext, ["phone_number_id"]),
    getValue(customerContext, ["phoneNumberId"]),
    latestInboundMessage?.phoneNumberId,
    latestOutboundMessage?.phoneNumberId
  );
  ...
  const phone = pickFirstString(
    getValue(customerContext, ["phone"]),
    getValue(customerContext, ["phone_number"]),
    getValue(root, ["phone"]),
    getValue(root, ["mobile"])
  );
  const idCustomer = pickFirstId(
    getValue(customerContext, ["id_customer"]),
    getValue(customerContext, ["idCustomer"]),
    getValue(customerCandidate.value, ["id_customer"]),
    getValue(customerCandidate.value, ["idCustomer"])
  );
  const idOrder = pickFirstId(
    getValue(customerContext, ["id_order"]),
    getValue(customerContext, ["idOrder"]),
    getValue(customerCandidate.value, ["id_order"]),
    getValue(customerCandidate.value, ["idOrder"])
  );
  const invoiceNumber = pickFirstId(
    getValue(customerContext, ["invoice_number"]),
    getValue(customerContext, ["invoiceNumber"]),
    getValue(customerCandidate.value, ["invoice_number"]),
    getValue(customerCandidate.value, ["invoiceNumber"])
  );
  const contactId = pickFirstId(
    getValue(customerContext, ["contact_id"]),
    getValue(customerContext, ["contactId"]),
    getValue(customerCandidate.value, ["contact_id"]),
    getValue(customerCandidate.value, ["contactId"])
  );
  ...
  const humanOwnershipActive = Boolean(
    caseContext &&
  ...
  const manualReplyActive = Boolean(
    caseContext &&
  ...
  const commercialIntentLegacy = pickFirstString(
    getValue(caseContext, ["commercial_intent_legacy"]),
    getValue(caseContext, ["service_code"]),
    getValue(caseContext, ["final_action"]),
    latestInboundMessage?.intent,
    latestOutboundMessage?.intent,
    getValue(serviceContext, ["service_code"]),
    getValue(serviceContext, ["primary_service"]),
    getValue(businessContext, ["postventa_queue", "estado_caso"]),
  ...
  const limitedRecentMessages = recentMessages.slice(0, COMMERCIAL_CONTEXT_MAX_RECENT_MESSAGES);
  ...
  return {
    ...
    context: {
      ...
      department,
      conversationCaseId: pickFirstId(
        getValue(root, ["conversation_case_id"]),
        getValue(root, ["conversationCaseId"]),
        getValue(caseContext, ["conversation_case_id"]),
        latestInboundMessage?.conversationCaseId
      ),
      waId,
      phoneNumberId,
      email,
...
export function normalizeCommercialBrainContext(input: unknown) {
  return normalizeCommercialContext(input);
}

export function normalizeCommercialInboundMessage(input: unknown): NormalizedCommercialInboundMessage {
  const normalized = normalizeMessage(input, "inbound");
  const sanitized = isRecord(input) ? sanitizeCommercialObject(input) : { value: null, applied: false, sanitizedFields: [] as string[] };
...
export function normalizeCommercialPolicyContext(input: unknown): SalesAgentPolicyContext | undefined {
  return sanitizePolicyContext(input);
}

export function buildCommercialStructuralSignals(input: {
  hasLatestCustomerMessage: boolean;
  hasCustomerCandidate: boolean;
  hasCustomerReference: boolean;
  hasConversationHistory: boolean;
  hasOrderReference: boolean;
  hasProductServiceContext: boolean;
  humanOwnershipActive: boolean;
  aiBlocked: boolean;
  manualReplyActive: boolean;
  hasCommercialEntity: boolean;
}): SalesAgentStructuralSignal[] {
  const signals: SalesAgentStructuralSignal[] = [];
  if (input.hasLatestCustomerMessage) signals.push("customer_message_present");
  if (input.hasCustomerCandidate) signals.push("customer_candidate_available");
  if (input.hasCustomerReference) signals.push("customer_reference_available");
  if (input.hasOrderReference) signals.push("order_reference_available");
  if (input.hasProductServiceContext) signals.push("product_service_context_available");
  if (input.hasConversationHistory) signals.push("conversation_history_available");
  if (input.humanOwnershipActive) signals.push("human_owner_active");
  if (input.aiBlocked) signals.push("ai_blocked");
  if (input.manualReplyActive) signals.push("manual_reply_active");
  if (input.hasCommercialEntity) signals.push("commercial_entity_available");
  return signals;
}

export function hasStaleCommercialContext(currentTimeIso: string, timestamps: Array<string | null | undefined>) {
  ...
  return currentTimeMs - latest > COMMERCIAL_CONTEXT_STALE_THRESHOLD_MS;
}

export function buildCommercialSourceSummary(input: CommercialContextSourceSummary): CommercialContextSourceSummary {
  return input;
}

export function buildCommercialIdentityContext(input: NormalizedCommercialBrainContext): SalesAgentIdentityContext {
  return {
...
export function buildCommercialMessageContext(input: {
  latestInboundMessage: SalesAgentMessageSnapshot | null;
  latestOutboundMessage: SalesAgentMessageSnapshot | null;
  recentMessages: SalesAgentMessageSnapshot[];
  latestInboundAt: string | null;
...
export function buildCommercialCaseContext(input: NormalizedCommercialBrainContext): SalesAgentCaseContext {
  return {
...
export function buildCommercialCommercialContext(input: NormalizedCommercialBrainContext): SalesAgentCommercialContext {
  return {
...
export function collectCommercialTimestampSignals(input: {
  latestInboundAt: string | null;
  latestOutboundAt: string | null;
  caseUpdatedAt?: string | null;
  customerCandidateUpdatedAt?: string | null;
  orderUpdatedAt?: string | null;
  productServiceUpdatedAt?: string | null;
}) {
  return [
...
export function normalizeCommercialCurrentTime(currentTime: string | Date) {
...
}): CommercialContextBuilderMetadata {
  return {
    version: COMMERCIAL_CONTEXT_VERSION,
    generatedAt: input.currentTime,
    currentTime: input.currentTime,
    timezone: input.timezone,
    requestedMode: input.requestedMode as CommercialContextBuilderMetadata["requestedMode"],
    availableCapabilities: [...input.availableCapabilities] as CommercialContextBuilderMetadata["availableCapabilities"],
