...
import type {
  AiSdrOperatorPilotBuildInput,
  AiSdrOperatorPilotCommercialState,
  AiSdrOperatorPilotControls,
  AiSdrOperatorPilotDiagnosticsLink,
  AiSdrOperatorPilotInvariants,
  AiSdrOperatorPilotKnownInformationItem,
  AiSdrOperatorPilotMissingInformationItem,
  AiSdrOperatorPilotNextAction,
  AiSdrOperatorPilotStatus,
  AiSdrOperatorPilotViewModel
} from "./types";
...
function sanitizeDisplayText(value: unknown, maxLength = MAX_TEXT_LENGTH): string | null {
...
function normalizeList(values: unknown, maxItems = 12): string[] {
  if (!Array.isArray(values)) return [];
  return uniqueStrings(values.map((value) => sanitizeDisplayText(value, 400))).slice(0, maxItems);
}

function extractCandidate(input: AiSdrOperatorPilotBuildInput): Record<string, unknown> | null {
...
function resolveStatus(input: AiSdrOperatorPilotBuildInput, candidate: Record<string, unknown> | null): { status: AiSdrOperatorPilotStatus; reason: string | null } {
  if (candidate) {
    ...
    if (status === "completed" || status === "available") return { status: "available", reason: null };
    if (status === "skipped" && skipReason === "skipped_by_flag") return { status: "disabled", reason: "commercial_operational_loop_disabled" };
    if (status === "skipped") return { status: "waiting_for_operational_loop", reason: "commercial_operational_result_skipped" };
    if (status === "disabled") return { status: "disabled", reason: "commercial_operational_loop_disabled" };
    ...
    if (status === "not_found") return { status: "not_found", reason: "commercial_operational_result_missing" };
...
function extractResultState(candidate: Record<string, unknown>) {
  return asRecord(candidate.resultingState) ?? asRecord(candidate.commercialState) ?? asRecord(candidate.state);
}

function extractSelectedNextAction(candidate: Record<string, unknown>) {
...
function buildCommercialState(candidate: Record<string, unknown>, status: AiSdrOperatorPilotStatus, review: CommercialShadowReviewViewModel): AiSdrOperatorPilotCommercialState | null {
  if (status !== "available") {
...
function buildKnownInformation(
  input: AiSdrOperatorPilotBuildInput,
  status: AiSdrOperatorPilotStatus,
  candidate: Record<string, unknown> | null
): AiSdrOperatorPilotKnownInformationItem[] {
  const review = input.commercialShadowReview;
  const items: AiSdrOperatorPilotKnownInformationItem[] = [];

  const push = (label: string, value: unknown, source: string, confidence: number | null) => {
...
function buildMissingInformation(
  ...
  const review = input.commercialShadowReview;
  const items: AiSdrOperatorPilotMissingInformationItem[] = [];

  const push = (key: string, label: string, reason: string | null, requiredFor: string | null) => {
    items.push({
      key,
      label,
  ...
  if (review.status === "available") {
    push(
      "operational_result",
      "Resultado operacional persistido",
      "Todavia no existe un resultado operacional duradero para este caso; la vista usa la observacion shadow como referencia parcial.",
    ...
    if (review.summary?.approvalRequirement && review.summary.approvalRequirement !== "none") {
      push(
        ...
        "La observacion shadow sugiere que el siguiente paso todavia requiere gobernanza o revision humana.",
        shadowActionLabel
      );
    }
    return items.slice(0, 8);
...
function humanizeNextActionType(value: string) {
...
function buildFallbackNextAction(
  input: AiSdrOperatorPilotBuildInput,
  candidate: Record<string, unknown> | null,
  status: AiSdrOperatorPilotStatus
): AiSdrOperatorPilotNextAction | null {
  const review = input.commercialShadowReview;
  ...
  if (candidate && status === "available") {
    ...
    const draftMessage = sanitizeDisplayText(
      selectedNextAction?.draftMessage ?? candidate.draftMessage ?? candidate.responseDraft ?? responseProposal?.draftText ?? responseProposal?.text,
      MAX_DRAFT_LENGTH
    );
...
function buildControls(status: AiSdrOperatorPilotStatus): AiSdrOperatorPilotControls {
  return {
    canApprove: false,
    canReject: false,
    canEditDraft: false,
...
function buildDiagnosticsLink(): AiSdrOperatorPilotDiagnosticsLink {
  return {
    available: true,
...
function buildInvariants(): AiSdrOperatorPilotInvariants {
  return {
...
function buildWarnings(review: CommercialShadowReviewViewModel, status: AiSdrOperatorPilotStatus, sourceReason: string | null): string[] {
...
function buildError(review: CommercialShadowReviewViewModel, status: AiSdrOperatorPilotStatus, sourceReason: string | null) {
  if (status !== "error") return null;
  return sanitizeDisplayText(
    review.error?.message ?? sourceReason ?? "No fue posible construir una vista operacional segura para este caso.",
    1200
  );
}

export function buildAiSdrOperatorPilotViewModel(input: AiSdrOperatorPilotBuildInput): AiSdrOperatorPilotViewModel {
