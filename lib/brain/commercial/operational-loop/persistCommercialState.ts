...
import type { CommercialOperationalPersistInput, CommercialOperationalPersistenceResult, CommercialOperationalState } from "./types";
...
async function safeHasTable(connection: PoolConnection, tableName: string) {
  try {
...
function buildOpportunityValues(state: CommercialOperationalState, currentTime: string) {
  return {
...
export async function persistCommercialState(input: CommercialOperationalPersistInput): Promise<CommercialOperationalPersistenceResult> {
  ...
  try {
    return await withConnection(async (connection) => {
      ...
      if (!opportunitiesTableExists || !decisionsTableExists) {
        ...
        } satisfies CommercialOperationalPersistenceResult;
      }

      await connection.beginTransaction();
      try {
        const [existingDecisionRows] = await connection.execute<RowDataPacket[]>(
          `SELECT id, opportunity_id FROM crm_agent_decisions WHERE decision_id = ? LIMIT 1`,
          [input.decisionRecord.decisionId]
        );
        const existingDecision = existingDecisionRows[0] as RowDataPacket | undefined;
        if (existingDecision) {
        ...
        const [existingOpportunityRows] = await connection.execute<RowDataPacket[]>(
          `SELECT id, version, status, stage, closed_at FROM crm_opportunities WHERE opportunity_key = ? LIMIT 1`,
          [input.resultingState.opportunityKey]
        );
        const existingOpportunity = existingOpportunityRows[0] as RowDataPacket | undefined;
        ...
        let opportunityId: string | number | null = existingOpportunity?.id ?? input.previousState?.opportunityId ?? null;
