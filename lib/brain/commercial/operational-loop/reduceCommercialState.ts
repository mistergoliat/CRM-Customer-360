import type { CommercialSignal, CommercialPriority, CommercialTemperature, OpportunityStage, OpportunityStatus } from "../types";
import type { CommercialOperationalLoopWarning } from "./constants";
import type {
  CommercialOperationalState,
  CommercialOperationalStateDiff,
  CommercialOperationalStateReductionInput,
  CommercialOperationalStateReductionResult
} from "./types";
...
function normalizeChannel(value: unknown): CommercialOperationalState["channel"] {
...
function diffArray<T>(previous: T[], next: T[]) {
...
function deriveStructuralSignals(input: CommercialOperationalStateReductionInput): CommercialSignal[] {
  const sourceSummary = input.commercialContext?.sourceSummary ?? null;
  const signals = new Set<CommercialSignal>();
  for (const signal of input.previousState?.signals ?? []) signals.add(signal);
  if (sourceSummary?.hasLatestCustomerMessage) signals.add("replied");
  if (sourceSummary?.orderContextAvailable) signals.add("shares_requirements");
  if (sourceSummary?.productServiceContextAvailable) signals.add("shares_requirements");
  if (sourceSummary?.humanOwnershipActive) signals.add("human_requested");
  if (sourceSummary?.aiBlocked) signals.add("rejection_explicit");
  if (sourceSummary?.manualReplyActive) signals.add("replied");
  if (input.salesAgentResult?.shouldRequestHuman) signals.add("human_requested");
  if (input.salesAgentResult?.shouldRequestTool) signals.add("shares_requirements");
  if (input.salesAgentResult?.shouldRespondNow) signals.add("replied");
  if (input.salesAgentResult?.decision.type === "wait_for_customer" || input.salesAgentResult?.outcome === "waiting_for_customer") signals.add("conversation_inactive");
  if (input.salesAgentResult?.decision.type === "blocked_by_policy" || input.salesAgentResult?.outcome === "blocked_by_policy") signals.add("rejection_explicit");
  return [...signals];
}

function derivePriority(previousState: CommercialOperationalState | null, signals: CommercialSignal[]): CommercialPriority {
  if (previousState && previousState.priority !== "normal") return previousState.priority;
  if (signals.includes("human_requested")) return "high";
  if (signals.includes("purchase_confirmed")) return "urgent";
  return "normal";
}

function deriveTemperature(previousState: CommercialOperationalState | null, signals: CommercialSignal[]): CommercialTemperature {
  if (previousState) return previousState.temperature;
  if (signals.includes("purchase_confirmed")) return "hot";
  if (signals.includes("human_requested")) return "warm";
  if (signals.includes("rejection_explicit")) return "cold";
  return "unknown";
}

function deriveStatus(previousState: CommercialOperationalState | null, input: CommercialOperationalStateReductionInput, signals: CommercialSignal[]): OpportunityStatus {
  ...
  if (!previousState) {
    if (signals.includes("human_requested")) return "stalled";
    if (signals.includes("replied")) return "engaged";
    return "new";
  }

  if (previousState.status === "new" && signals.includes("replied")) return "engaged";
  if (previousState.status === "engaged" && (signals.includes("human_requested") || input.salesAgentResult?.outcome === "tool_required" || input.salesAgentResult?.decision.type === "request_tool")) return "qualifying";
  if (previousState.status === "qualifying" && (input.salesAgentResult?.outcome === "tool_required" || input.salesAgentResult?.decision.type === "request_tool")) return "quote_pending";
  if (previousState.status === "qualifying" && (input.salesAgentResult?.outcome === "waiting_for_customer" || input.salesAgentResult?.decision.type === "wait_for_customer")) return "waiting_customer";
  if (previousState.status === "quote_pending" && (input.salesAgentResult?.outcome === "waiting_for_customer" || input.salesAgentResult?.decision.type === "wait_for_customer")) return "waiting_customer";
  if (previousState.status === "waiting_customer" && signals.includes("replied")) return "engaged";
  if (previousState.status === "stalled" && signals.includes("replied")) return "engaged";

  return previousState.status;
}

function deriveStage(status: OpportunityStatus): OpportunityStage | null {
...
function buildBaseState(input: CommercialOperationalStateReductionInput, previousState: CommercialOperationalState | null, signals: CommercialSignal[]): CommercialOperationalState {
  const now = toIsoString(input.currentTime);
  const summary =
    input.salesAgentResult?.analysis.summary ??
...
function buildStateDiff(previousState: CommercialOperationalState | null, resultingState: CommercialOperationalState): CommercialOperationalStateDiff {
  ...
  return {
    ...
    changedFields,
    addedSignals: signalDiff.added as CommercialSignal[],
    removedSignals: signalDiff.removed as CommercialSignal[],
...
export function reduceCommercialState(input: CommercialOperationalStateReductionInput): CommercialOperationalStateReductionResult {
  ...
  const resultingState: CommercialOperationalState = {
    ...baseState,
    ...
    waitingFor:
      input.commercialPolicyResult?.status === "blocked" || input.commercialPolicyResult?.requiresApproval === "blocked"
        ? "operator_review"
  ...
  const reason =
    input.commercialPolicyResult?.status === "blocked"
