import { validateFollowUpMutationPlan } from "./validateMutationPlan";
import type {
  FollowUpMutationApplyResult,
  FollowUpMutationMemoryAction,
  FollowUpMutationMemoryState,
  FollowUpMutationPlan
} from "./types";

function cloneState(state: FollowUpMutationMemoryState): FollowUpMutationMemoryState {
  return JSON.parse(JSON.stringify(state)) as FollowUpMutationMemoryState;
}

function normalizeState(state: FollowUpMutationMemoryState): FollowUpMutationMemoryState {
  return {
...
function findActionIndex(actions: FollowUpMutationMemoryAction[], actionId: string) {
  return actions.findIndex((action) => action.actionId === actionId);
}

function isDuplicateActionId(state: FollowUpMutationMemoryState, actionId: string) {
  return state.actions.some((action) => action.actionId === actionId);
}

function isDuplicateIdempotencyKey(state: FollowUpMutationMemoryState, idempotencyKey: string) {
  return state.actions.some((action) => action.idempotencyKey !== null && action.idempotencyKey === idempotencyKey);
}

export function applyFollowUpMutationPlanInMemory(
  state: FollowUpMutationMemoryState,
  plan: FollowUpMutationPlan
): FollowUpMutationApplyResult {
